...
since_tms= time.time() - tianshu_tms
# 2023-12-25T02:13:42PST+0800
since_time =time.strftime("%Y-%m-%dT00:00:00%z", time.localtime(since_tms))
# 2023-12-25T02:13:42PST+0800
until_time = time.strftime("%Y-%m-%dT%H:%M:%S%z", time.localtime())

# print('time.time()',time.time())
# print('tianshu_tms',tianshu_tms)
# print('since_tms',since_tms)
...
#cmd = 'git log --format=\'%aN\' | sort -u | while read name; do echo -en "$name\t"; git log --author="$name" --pretty=tformat: --numstat | awk \'{ add += $1; subs += $2; loc += $1 - $2 } END { printf "added lines: %s, removed lines: %s, total lines: %s\n", add, subs, loc }\' -; done'
# cmd = "git log --format=\'%aN\' | sort -u"
cmd = "git log --since="+since_time+" --until="+ until_time+"  --format='%aN' | sort -u"
...
print_and_write('')
print_and_write('')
print_and_write('统计分支为：HEAD/'+branch)
