        ...
        <template v-if="presettleorderdetail_data.length">
          <div class="detail-wrap yb_pb12" v-for="(item,index) in presettleorderdetail_data" :key="index">
            ...
            <template v-if="item.orderStatus == 2">
              <!-- 部分结算 -->
              <!-- 结算本金 -->
              <p class="yb_mt4"><span>{{item.remainingBetAmount?t('early.list7'):t('early.list2')}}</span><span>0.00</span></p>
              <!-- 返还金额 -->
            ...
            <template v-else>
              <!-- 部分结算 -->
              <!-- 结算本金 -->
              <p class="yb_mt4"><span>{{item.remainingBetAmount?t('early.list7'):t('early.list2')}}</span><span>{{(+item.preBetAmount).toFixed(2)}}</span></p>
              <!-- 返还金额 -->
