      ...
      <scroll ref="myScroll" :on-pull="onPull" v-else>
        <div class="edit row items-center yb_fontsize12">
          <div class="time yb_mr6 relative-position" @click="change_date">
            <i class="calendar"></i><span>{{ date_limit == 7 ? $root.$t('bet_record.7day') :
              $root.$t('bet_record.30day') }}</span>
          </div>
          <div class="sort relative-position" @click.stop="change_sort($event)">
            <i :class="'sort-' + sort_active"></i>
            <span>{{ sort_active == 2 ? $root.$t('bet_record.sort0') : sort_active == 1 ? $root.$t('bet_record.sort1') :
              $root.$t('bet_record.sort2') }}</span><span></span>
            <!-- 默認排序 -->
            <!-- 按投注时间排序 -->
            <!-- 按开赛时间排序 -->
            <p v-if="is_sort_show && 0" class="absolute">
              ...
              <span class="sort-text" :class="{ 'select': sort_active == 1 }" data-num='1'><i class="sort1"
                  :class="{ 'sort-1': sort_active == 1 }"></i>{{ $root.$t('bet_record.sort4') }}</span>
              <span class="sort-text" :class="{ 'select': sort_active == 3 }" data-num='3'><i class="sort2"
                  :class="{ 'sort-3': sort_active == 3 }"></i>{{ $root.$t('bet_record.sort5') }}</span>
            </p>
          </div>
          <div>
                  ...
                  <span class="betamount" v-show="store_cathectic.main_item == 1 && value.open">{{
                    $root.$t('bet.number_transactions') }}<span class="color-1 yb_m">{{ value.totalOrders }}</span>&emsp;{{
                      $root.$t('bet.betting') }}<span class="color-1">{{ value.betAmount }}</span>&emsp;{{
                        $root.$t('bet_record.bet_no_status03') }}/{{ $root.$t('bet_record.bet_no_status04') }}<span
