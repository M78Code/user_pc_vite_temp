...
import { normal_img_not_favorite_white, normal_img_not_favorite_black, normal_img_is_favorite, y0_img_favorite_black, lvs_icon_theme01, lvs_icon_theme02, 
  animationUrl_icon_theme01, animationUrl_icon_theme02, muUrl_theme01, muUrl_theme01_y0, muUrl_theme02, muUrl_theme02_y0 } from 'src/boot/local-image'
...
// 监听客队红牌比分变化
watch(() => away_red_score, (new_,old_) => {
  if(match.csid != 1) return
...
const animationUrl_icon = computed(() => {
  let is_theme01 = get_theme.includes('theme01')
  // let is_theme02 = get_theme.includes('theme02')
  // let is_y0 = get_theme.includes('y0')
...
const get_sport_show = computed(() => {
  ...
  if (!_.get(get_current_menu, 'main.menuType')) {
    if (props.i > 0) {
      ...
      if(_.get(get_hot_tab_item,'index')!=0){return false}
      return true;
    }
...
const favorited_computing_icon = computed(() => {
  let flag = null
  if(!_.get(get_user, 'favoriteButton') && get_theme.includes('y0')){
...
const no_start_total = computed(() => {
  ...
  if(match.is_show_no_play){
    r = props.matchCtr.match_list_data_sources.length - (props.i);
  }
  return r;
})
...
const default_league_icon = computed(() => {
  return get_theme.includes('theme02') ? none_league_icon_black : none_league_icon
})

const show_newer_edition = computed(() => {
  return get_newer_standard_edition == 1 || props.main_source == 'detail_match_list';
})
...
set_is_in_play(final_button_type);
goto_details(match);
}
/**
 * 计算真正回落的点击按钮   直播 视频  动画
...
// 正常的 优先级 ： lvs 直播   muUrl 视频  animationUrl 动画
if (button_type == "lvs") {
  if (state_obj.lvs && ['string', 'number'].includes(typeof _.get(match,'lss')) ) {
    // 赛前图标
    if(_.get(match,'lss') == 1){
      state_obj.icon_path = lvs_icon_theme01
    }else if(_.get(match,'lss') == 0 && _.get(match,'ms') != 1){
      // 正在直播的图标
...
const is_show_result = () => {
  let r = false;
  if(_.get(get_current_menu, 'main.menuType') == 28){
...
const need_scroll_height_handle = (tid) => {
  ...
  // 赛事列表
  const match_list = _.cloneDeep(props.matchCtr.match_list_data_sources)
  // 赛事dom块高度映射列表
  const match_height_map_list = _.cloneDeep(props.matchCtr.view.match_height_map_list)
  // 展开折叠 未开赛的联赛后 需滚动的距离
...
const goto_details = (match, flag) => {
  ...
  // 进入详情页前，记录目标赛事top值
  set_goto_detail_match_info({top: $refs['mid-' + match.mid].getBoundingClientRect().top});
  set_not_found_target_dom_count(0);
  set_details_item(0);
  // 进入详情前，将当前赛事信息存入仓库
  set_match_base_info_obj(match)
...
const score_value = () => {
  ...
  if(!match.away_red_score && match.away_red_score != 0){
    ...
    away_red_score = match.away_red_score;
  }
  ...
  if(get_menu_type !=28 && match.csid==1 && match.cds=='RC' && get_newer_standard_edition == 1 && footer_sub_menu_id == 114){
  ...
  $nextTick(()=>{
    scroll_top_timer = setTimeout(() => {
      ...
      if (mid_dom) {
        ...
        // 目标赛事视图top阈值
        let view_top = _.get(get_goto_detail_match_info, 'top', 0)
        ...
        if (view_top) {
          ...
          // 短距离滚动标识
          set_allow_short_scroll(true)
          // 已滚动至目标dom时，未滚动至目标计数置为-1
          set_not_found_target_dom_count(-1)
        ...
        if (not_found_target_dom_count >= 0) {
          ...
          set_not_found_target_dom_count(not_found_target_dom_count)
