...
export default defineComponent({
  ...
  setup(props, evnet) {
    ...
    onMounted(() => {
      ...
      // $root.$on(MITT_TYPES.EMIT_BASKETBALL_TIME_ARRIVED,basket_ball_time_handle);
      // $root.$on(MITT_TYPES.EMIT_FORCE_END_PLAYING_BASKETBALL,end_playing_basketball_handle);
      // $root.$on(MITT_TYPES.EMIT_INGAME_RESULT_SHOW_END,ingame_result_show_end);
    })
    onUnmounted(() => {
      emitters.map((x) => x())
    })
