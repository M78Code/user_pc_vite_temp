...
<template>
  <div class="group-rank">
    ...
    <div class="header">
      <div class="col1"></div>
      <div class="col2">{{ i18n.t('virtual_sports.team') }}</div>
      <div class="col3">{{ i18n.t('virtual_sports.game') }}</div>
      <div class="col4">{{ i18n.t('virtual_sports.win_tie_loss') }}</div>
      <div class="col5">{{ i18n.t('virtual_sports.advance') }}</div>
      <div class="col6">{{ i18n.t('virtual_sports.lose') }}</div>
      <div class="col7">{{ i18n.t('virtual_sports.goal_difference') }}</div>
      <div class="col8">{{ i18n.t('virtual_sports.integral') }}</div>
    </div>
    <!-- 小组 -->
    <div class="group-item hairline-border" v-for="(detail_list,index) in group_stage_list" :key="index" v-show="!no_data">
      <div class="group-name">{{get_lang == 'vi' ? (i18n.t('virtual_sports.group')+ '  ' + detail_list.groupId) : (detail_list.groupId+ '  ' +$t('virtual_sports.group'))}}</div>
