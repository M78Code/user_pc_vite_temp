<template>
  <div class="group-rank">
    ...
    <div class="header">
      ...
      <div class="col5">{{ $root.$t('virtual_sports.advance') }}</div>
      <div class="col6">{{ $root.$t('virtual_sports.lose') }}</div>
      <div class="col7">{{ $root.$t('virtual_sports.goal_difference') }}</div>
      <div class="col8">{{ $root.$t('virtual_sports.integral') }}</div>
    </div>
    <!-- 小组 -->
    <div class="group-item hairline-border" v-for="(detail_list,index) in group_stage_list" :key="index" v-show="!no_data">
      <div class="group-name">{{get_lang == 'vi' ? ($root.$t('virtual_sports.group')+ '  ' + detail_list.groupId) : (detail_list.groupId+ '  ' +$t('virtual_sports.group'))}}</div>
