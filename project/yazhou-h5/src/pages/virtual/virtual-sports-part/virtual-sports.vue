  ...
  // },
    ...
    watch(
      ...
      (change_str) => {
        ...
        tab_item_i = 0;
        ranking_list_change = is_show_analyse;
        // 根据 足蓝标识设置二级菜单切换状态
        top_menu_changed = !change_str.includes('zu_lan_')
      }
    );
    watch(
      () => props.menu_list,
      () => {
        const { tab_item_i } = data;
        if(tab_items && tab_items.length){

        if(current_league){
          tab_items.forEach((t_item,i) => {
    ...
    watch(
      ...
      () => {
        ...
        tab_item_click_handle(data.tab_item_i);
    ...
    onMounted(() => {
      ...
      // $root.$on(MITT_TYPES.EMIT_ARRIVED10,arrived10_handle);
      // $root.$on(MITT_TYPES.EMIT_MATCH_EDNED_STATUS2,match_ended_status2_handle);
      match_ended_status2_handle();
    });

    onUnmounted(() => {
      is_before_destroy = Math.random();
      if(video_process_obj && video_process_obj.destroy){
        video_process_obj.destroy();
      }
      emitters.map((x) => x())
      // $root.$off(MITT_TYPES.EMIT_ARRIVED10,arrived10_handle);
      // $root.$off(MITT_TYPES.EMIT_MATCH_EDNED_STATUS2,match_ended_status2_handle);
    ...
    const arrived10_handle = ({mid,batchNo}) => {
      if(match_list_by_no && match_list_by_no.length){
        match_list_by_no.forEach(m => {
          if(m.batchNo == batchNo){
            m.mhs = 1;
          }
        });
      }
      if(sub_menu_type == 1004 && !singleton_10second){
        singleton_10second = true;
    ...
    const match_ended_status2_handle = () => {
      ...
      for (let i = 0; i < match_list_by_no.length; i++) {
        const m = match_list_by_no[i];
      ...
      if(sub_menu_type == 1004){
        if(flag == 'is_basketball_playing'){
          get_video_process_by_api(() => {
            get_match_video_process(current_match);
          });
    ...
    const tab_item_click_handle = (i,is_force,user_change) => {
      if(tab_item_i === i && !is_force) return;
      if(ranking_list_change) {
        before_match_tab_trend = Math.random();
      }
      if(user_change == 'user_change'){
        set_current_batch({});
      }
      tab_item_i = i;
      if(tab_items && tab_items.length && tab_item_i > -1){
        let current_league = tab_items[tab_item_i];
        if(!current_league){
