  ...
  setup(props, evnet) {
    ...
    onMounted(() => {
      ...
      if(current_match.mmp == 'PREGAME'){
        ...
        if(v_m_status == 2){
          ...
          timer2 = setTimeout( () => {
            // #TODO 
            // $root.$emit(emit_cmd.EMIT_INGAME_RESULT_SHOW_END);
          },14000)
        }
      }
    });
    ...
    const animation_b = () => {
      ...
      if(start_time < 6000){
        ...
        match_list.forEach( match => {
          let score = _.get(basketball_score,`${match.mid}.score`) || [0,0]
          match.home = Math.ceil(basketball_line_width / 100 * score[0])
          match.away = Math.ceil(basketball_line_width / 100 * score[1])
        })
      }else{
    ...
    */
    ...
    const set_match_list = () => {
      let match_list = []
      virtual_match_list.forEach( match => {
        match_list.push(_.clone(match))
      })
      match_list = match_list
    };
    const set_match_ended_m = () => {
      if(virtual_match_list){
        match_ended = virtual_match_list[0];
      }
      else{
        match_ended = current_match;
      }
      timer6 = setTimeout(() => {
        match_ended = null;
      },4000);
    };
    const get_process_stage = () => {
      //开赛后6秒
      if(started_second < 6){
        // m_status = 0;
      }
      //开赛后6到10秒
      else if(started_second >= 6 && started_second < 10){
        // m_status = 1;
      }
      else
      {
