...
export default defineComponent({
  ...
  setup(props, evnet) {
    const data = reactive({
      tabs: [
        // #TODO EMIT 
        // {name: $root.$t('virtual_sports.top_16'), key: 'Q8'},
        // {name: $root.$t('virtual_sports.quarter_finals'), key: 'Q4'},
        // {name: $root.$t('virtual_sports.semifinals'), key: 'SEMIFINAL'},
        // {name: $root.$t('virtual_sports.finals'), key: 'FINAL'}
      ],
      tab_index: -1,
      visible: false,
      no_data: false,
    ...
    const tabClick = (item, i) => {
      ...
      if (!Object.keys(check_if_there_tab).includes(item.key)) {
        $toast($root.$t('virtual_sports.no_data') + $root.$t(`${item.name}`), 1000)
        return
      }
