<template>
  <div class="vir-bask">
    ...
    <div class="vir-bask-head" v-if="current_match.mmp == 'INGAME' && !is_end">
      ...
      <div>{{$root.$t('virtual_sports.match_status.playing')}}</div>
    </div>
    <div class="vir-bask-head vir-end" v-if="current_match.mmp != 'INGAME'" v-html="$root.$t('virtual_sports.match_status.x_to_ended').replace('%s',basketball_end_time)"></div>
    <!-- 进度区域 -->
    <div class="progress" v-if="!is_end">
      <!-- 中文进度 -->
      <div class="row text-pro">
        <div class="col">{{$root.$t('virtual_sports.match_status.0')}}</div>
        <div class="col">{{$root.$t('virtual_sports.match_status.1')}}</div>
        <div class="col">{{$root.$t('virtual_sports.match_status.2')}}</div>
      </div>
      <!-- 线性进度 -->
      <div class="line-wrap">
    ...
    <!-- 比赛结束 -->
    <div v-if="is_end" class="vir-ending">{{$root.$t('virtual_sports.match_status.ended')}}</div>
    <!-- 对阵区域 -->
