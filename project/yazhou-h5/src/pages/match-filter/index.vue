...
<template>
  <div class="boss-box">
          ...
          <q-slide-transition>
            <div v-if="!item1.hide">
              <div  :key="index+'League-name'" class="row  items-center content_box1" >
                <div class="row justify-between items-center content_box2" :class="{'content_box3':item1.title && item1.spell}" @click.stop.prevent="select_li_ctr(item1)">
                  <div class="row items-center">
                    <div class="row items-center">
                      <!-- 联赛icon -->
                      <img :src=" item1.picUrlthumb ? get_file_path(item1.picUrlthumb) : get_theme.includes('theme02') ? none_league_icon_black : default_url" @error="league_icon_error" class="match_logo" />
                      <div class="name-overhide">{{ item1.nameText }}</div>
                      <div class="nums" v-show="!(type == 28  && [1001,1002,1004,1011,1010,1009].includes(get_curr_sub_menu_type))">{{item1.num}}</div>
                    </div>
                  </div>
                  <img v-if="item1.select" class="icon-search" :src="`${ $g_image_preffix }/image/bw3/svg/check_circle_outline-24px${on_suffix}.svg`" />
                  <img v-else :src="`image/wwwassets/bw3/svg/selected-no${get_theme.includes('02') ? '-2' : get_theme.includes('y0') ? '_y0' : ''}.svg`" alt="" class="icon-search" >
                </div>
              </div>
            </div>
          </q-slide-transition>
    ...
    <div class="allCheck row justify-between items-center" v-if="change && !list_data_loading">
      <div class="row items-center" :style="{lineHeight: ['vi','en','th','ms','ad'].includes(get_lang) ? '1' : 'unset'}">
        <template>
    ...
    <!-- 无数据展示 -->
    <no-data which="noMatch" style="margin-top: 0.26rem" :height="100" v-if="!list_data_loading && no_find_content"></no-data>
  </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        dom_: null, // dom操作对象
        get_sport_all_selected:false
      };
    },
    created() {
      this.timer2=0; // 计时器对象
      // 当前二级菜单选中的菜单类型
      // this.type = this.$store.state.topmenu.menu_type||1;
      // this.get_top();
    ...
    computed: {
      ...
      // ]),
      all_checked(){
        return this.list.every(({select})=>select); // 选中所有
      },
      on_suffix(){
        let suffix='';
    ...
    methods: {
       ...
       */
      // default_selected() {
      //   this.list.forEach(i=>i.select=false);
      //   this.$forceUpdate()
      // },
      get_top() {
      ...
      all_checked_click() {
        ...
        const check_value=!this.all_checked;
        ...
        // 如果是 全选
        if(this.all_checked){
          this.select_num = number_of_filters;
        }else{// 如果不是 全选
          this.select_num = 0;
        }
      },
      ...
      select_btn_click() {
        ...
        if(this.all_checked) {
          ...
          this.select_num = cumulative_quantity
        }
        this.$forceUpdate(); //强制刷新页面，解决页面不会重新渲染的问题
      },
      // @Description:单个选择
      select_li_ctr(li_item) {
        if(li_item.select){
      ...
      fetch_filter_match() {
        ...
        api_match_filter(params).then(({ code, data }) => {
          try {
            ...
            //   this.list = data || [];
            // }
            this.list = (data || []).map(i=>({...i,select:i.id in this.selected})); // 初始化select
