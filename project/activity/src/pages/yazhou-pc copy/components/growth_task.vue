...
<template>
  <div class="tabs_content">
    ...
    <div class="activity_rules text-gray">
      ...
      <p v-if="isGrow">
        每日在本场馆累计投注<span>≥100</font>元，即视为投注1天；
      </p>
      <p v-else>
        单笔注单投注<span>≥100</font>元，方可视为每日任务活动有效注单；
      </p>
      <p v-if="isGrow">
        成长任务数据每小时更新一次，在下个自然周/自然月数据将自动清零重新计算，请会员于自然周期最后一天提前<span>一小时</font>完成任务并领取奖券，避免因数据延迟导致领取失败；
      </p>
      <p v-else>
        每日任务数据每<span>5</font>分钟更新一次，在次日数据将自动清零重新计算，请会员每日提前<span>30</font>分钟完成任务并领取奖券，避免因数据延迟导致领取失败；
      </p>
      <p v-if="isGrow">
        成长任务活动有效注单以结算时间为准。任何低于欧洲盘<span>1.5</font>(香港盘<span>0.5</font>)水位、同场赛事中投注对等盘口、串关注单，皆不予计算；
      </p>
      <p v-else>
        每日任务活动有效注单以结算时间为准，且需满足单笔投注金额<span>≥100</font>元。任何低于欧洲盘<span>1.5</font>(香港盘<span>0.5</font>)水位、同场赛事中投注对等盘口、串关注单，皆不予计算；
      </p>
      <p>
