/*
 * @Author: Amor
 * @Date: 2020-09-09 12:31
 * @Description: 赛事列表通用逻辑处理
 */
...
const match_list = {
  ...
  watch: {
    ...
    "match_list_card.match_list_card_key_arr":{
      handler(cur,old){
        // 当收藏列表没数据的时候跳转列表页11
        // console.warn(cur.length,'curcurcurcurcurcurcur');
        //&& _.get(this.match_list_data, 'match_list.length')==0
        if (this.vx_layout_list_type === 'collect' && !cur ) {
          this.vx_set_layout_list_type("match");
          // 获取赛事列表数据
          this.fetch_match_list()
          // 设置联赛吸顶高度
  ...
  methods: {
     ...
     */
       regular_events_set_match_details_params(cut,params){
         const match_info = this.match_list_data.mid_obj['mid_'+ params.mid]
         // 新手版的话首次进入有视频优先播放视频
         if (this.get_version==2) {
          let { lvs = -1, mms = -1, lss = -1, tvs = -1, ms, varl = "", vurl = "",mvs } = match_info

               let is_play = this.$utils.get_match_status(ms)
               if (mms == 2 && (varl || vurl || !$menu.menu_data.is_esports) && is_play) {
                params.media_type='video'
               }else if(mvs>-1){
                params.media_type='animation'
               }else{
                params.media_type='auto'
               }
         }
      ...
      if (code == 200 && all_league_list.length > 0) {
        ...
        if(backend_run){
          ...
          if($menu.menu_data.match_tpl_number == 18){
            ...
            //   setTimeout(() => {
            //     console.log(11111111,this.match_list_data.mid_obj)
            //     const match_info = this.match_list_data.mid_obj['mid_'+ mids[0]]
            //     console.log(11111111,match_info)
            //     let { lvs = -1, mms = -1, lss = -1, tvs = -1, ms, varl = "", vurl = "",mvs } = match_info
   ...
   */
    ...
    mx_collect_count(data) {
      ...
      // 赛事收藏数量去除联赛id因选择联赛时会携带联赛id导致收藏数量不正确
      delete _params.tid
      // 电竞
      if($menu.menu_data.is_esports){
