 ...
 * @Description: 单关投注信息组件 minxin  正常
 */
...
export default {
  ...
  data() {
    return {
      ...
      get_bet_pre_timer: null, // 获取预约限额节流配合接口限频
    };
  },
  props: {
    view_ctr_obj: { //数据源
  ...
  created() {
    ...
    this.$root.$on(this.emit_cmd.EMIT_SINGLE_UPDATE_KEYBOARD_STATUS_CMD, this.update_keyboard_status);// 更新键盘状态
    this.$root.$on(this.emit_cmd.EMIT_UPDATE_HOME_AWAY_CMD, this.update_home_away);//更新主客队信息(主要用于国际化切换时调用)
    this.$root.$on(this.emit_cmd.EMIT_BET_SET_MONEY, this.set_money); // 设置输入框
    this.$root.$on(this.emit_cmd.EMIT_INIT_BET_LIST_ITEM_CMD, this.init_bet_list_item);// 初始化联赛名称 赛季 玩法名称 队伍名称
    this.$root.$on(this.emit_cmd.EMIT_CLOSE_BIG_VIDEO_BET, this.del_bet_item); //关闭大视频投注
    this.$root.$on(this.emit_cmd.EMIT_NET_ERR, this.net_err_fun)//无网络事件
    ...
    //初始化后台开关权重按顺序排列
    this.marketConfigValue = _.get(this.vx_get_user, 'configVO.marketConfigValue', 0);
    this.bookBet = _.get(this.vx_get_user, 'configVO.bookBet', 0);
  ...
  destroyed() {
    ...
    this.$root.$off(this.emit_cmd.EMIT_SINGLE_UPDATE_KEYBOARD_STATUS_CMD, this.update_keyboard_status);
    this.$root.$off(this.emit_cmd.EMIT_UPDATE_HOME_AWAY_CMD, this.update_home_away);
    this.$root.$off(this.emit_cmd.EMIT_BET_SET_MONEY, this.set_money);
    this.$root.$off(this.emit_cmd.EMIT_INIT_BET_LIST_ITEM_CMD, this.init_bet_list_item);
    this.$root.$off(this.emit_cmd.EMIT_CLOSE_BIG_VIDEO_BET, this.del_bet_item);
     //取消防抖和节流
  ...
  computed: {
    ...mapGetters({
      vx_get_bet_single_list: "get_bet_single_list",// 单关信息列表
      vx_get_bet_single_obj: "get_bet_single_obj", // 单关对象扁平化
      vx_get_user: "get_user",//用户信息
      vx_is_bet_single: "is_bet_single",//是单关
      vx_cur_menu_type: "get_cur_menu_type",//获取当前菜单
      get_is_fullscreen: "get_is_fullscreen",//是否全屏
      vx_get_cur_odd: "get_cur_odd", //当前赔率
      vx_get_lang_change: "get_lang_change", // 是否改变语言

      vx_get_bet_appoint_obj: "get_bet_appoint_obj",// 被预约的投注项id

      vx_get_is_bet_merge: "get_is_bet_merge",//是否单关合并
      lang: "get_lang",// 当前语言
    ...
    match_type() {
      // 默认为普通赛
      return _.get(this.vx_get_bet_single_obj, `${this.id}.cs.match_type`);
    },
    /**
     ...
     */
    market_type() {
      let match_type = _.get(this.vx_get_bet_single_obj, `${this.id}.cs.match_type`);
      if(match_type!=3) {
        return _.get(this.vx_get_bet_single_obj, `${this.id}.cs.market_type`);
      }
      return '';
    },
    /**
     ...
     */
    has_handicap_value() {
      let bet_obj = this.vx_get_bet_single_obj[this.id];
      if(bet_obj) {
     ...
     */
    handicap_name() {
      let cs = _.get(this.vx_get_bet_single_obj,`${this.id}.cs`);
      let odds_table = {
        EU: '1',//欧盘
        HK: '2',//香港盘
        MY: '3',
      ...
      if(this.source == 'is_chat_room'){
        _odd = this.vx_get_chat_room_type;
        this.vx_set_cur_odd(_odd);
        return `[${this.$root.$t('odds')[_odd]}]`;
      }
     ...
     */
    min_money() {
      let min_money =  _.get(this.vx_get_bet_single_obj,`${this.id}.cs.min_money`, '');
      return `${min_money}`
    },
    /**
     * @description: 最大值
     ...
     */
    max_money() {
      let max_money =  _.get(this.vx_get_bet_single_obj,`${this.id}.cs.max_money`, '');
      return `${max_money}`;
    },
    /**
     * @description:赛事时间
     ...
     */
    win_money() {
      let bet_obj = this.vx_get_bet_single_obj && this.vx_get_bet_single_obj[this.id];
      let temp = 0.00;
        ...
        if(this.appoint_odds_value==null) {
          ...
          if (hsw && hsw != '') {
            ...
            if (support_odds.includes(_.get(this,'vx_get_cur_odd'))) {
              //最终盘口类型
              marketTypeFinally = _.get(this,'vx_get_cur_odd');
            } else {
              //最终盘口类型
  ...
  watch: {
    ...
    computed_appoint_ball_head(new_) {
        let obj = _.cloneDeep(this.vx_get_bet_appoint_obj);
        ...
        obj.is_head_eq_hadicap = this.appoint_ball_head == this.init_ball_head; // 球头和盘口值是否一样
         // 设置预约投注项
        this.vx_set_bet_appoint_obj(obj);
        return obj;
    },
    ...
    appoint_odds_value(new_) {
      //  赔率大于355给个提示
       if (this.appoint_odds_value>355) {
        this.$root.$emit(this.emit_cmd.EMIT_SHOW_TOAST_CMD,`${this.$root.$t('bet.bet_max_booked_odds')}`);
      }
      let obj = _.cloneDeep(this.vx_get_bet_appoint_obj);
      if(obj) {
    ...
    money(new_,old){
      // 监听当是合并项的时候给money赋值
      if (new_===null&& this.vx_get_is_bet_merge) {
        this.money = this.vx_get_bet_current_money_obj[this.id]||null
      }
    }
  },
  beforeDestroy () {
  //  将输入框金额保存在vuex 待切换成小框使用
    if (this.money&&this.vx_get_bet_single_list.length>0) {
  ...
  methods: {
    ...mapActions({
      ...
      vx_bet_single_obj_remove_attr: "bet_single_obj_remove_attr", //移除单关投注项对象
      vx_bet_single_list_remove: "bet_single_list_remove",//移除单关列表
      vx_set_is_single_handle: 'set_is_single_handle',//设置单关是否处理中
      vx_set_layout_left_show: "set_layout_left_show", //设置左侧显示内容类型
      
      vx_set_bet_appoint_obj: "set_bet_appoint_obj", // 设置预约投注项

      vx_bet_single_clear: "bet_single_clear",  // 清除单关数据
      
      vx_set_pre_min_odd_value: "set_pre_min_odd_value", // 设置输入框最小值
     ...
     */
    net_err_fun(){
      let obj = _.cloneDeep(_.get(this.vx_get_bet_single_obj, `${this.id}`));
      if(!obj) return;
      obj.key = this.id;
    ...
    min_max_handle(bet_single_obj, data) {
      ...
      this.$root.$emit(this.emit_cmd.EMIT_SINGLE_UPDATE_KEYBOARD_STATUS_CMD); //更新键盘状态
    },
    /**
      ...
      this.query_pre_bet_amount((code, betAmountInfo) => {
        if(200 == code) {
          ...
          if(code == 200 && betAmountInfo && betAmountInfo.length > 0) {       
            let bet_single_obj = _.cloneDeep(this.vx_get_bet_single_obj);       
    ...
    keyup_handle(event) {
      ...
      if (this.money == '0' && event.keyCode == 8) {
        this.money = null;
        this.view_ctr_obj.single_range_money = 0;
      }
      // 金额存在
    ...
    init_bet_list_item() {
      ...
      // 队伍名称
      this.team_name = this.yabo_common.get_team_name(this);
      let bs = _.get(this.vx_get_bet_single_obj,`${this.id}.bs`);
      let cs = _.get(this.vx_get_bet_single_obj,`${this.id}.cs`);
    ...
    del_bet_item() {
      ...
      if(this.vx_get_bet_appoint_obj && this.vx_get_bet_appoint_obj.bet_appoint_id==this.id) {
         // 置空预约投注项
    ...
    set_bet_obj_value(k, v) {
      if (!this.vx_get_bet_single_list.includes(this.id)) {
        return;
      }
      let bet_obj = this.vx_get_bet_single_obj && this.vx_get_bet_single_obj[this.id];
      if (bet_obj) {
    ...
    check_money(value) {
      try {
        ...
        this.vx_get_bet_single_list.forEach(item => {
          let cs = this.vx_get_bet_single_obj && this.vx_get_bet_single_obj[item] && this.vx_get_bet_single_obj[item].cs;
          // 存储的金额为空并且输入金额不为0
        ...
        // 所有的投注项都没有输入金额时
        if (empty_count == this.vx_get_bet_single_list.length) {
          // 设置限额标识为输入为空
          this.view_ctr_obj.single_range_money = -2;
      ...
      if(this.active != 2 && this.active != 3 && count == 0) {
        ...
        if(delay) {
          ...
          this.timer_obj[`timer_${this.id}`] = setTimeout(() => {
            clearTimeout(this.timer_obj[`timer_${this.id}`]);
            // 复位盘口及赔率的变换标志
            this.yabo_common.reset_odds_handicap_change(this);
            // 重置提示信息
            this.yabo_common.init_message(this);
    ...
    update_keyboard(value) {
      _.forEach(this.keyboard_data, item => {
        ...
        if(item.value>value || (item.text=='MAX' && this.vx_get_is_bet_merge)) {
          item.disabled = true;
        } else {
          // 按键正常状态
          item.disabled = false;
    ...
    set_min_money() {
      if(this.money !== null) {
        ...
        localStorage.setItem("common_amount", this.money);
        // 设置提示信息
        this.yabo_common.check_result_msg(this, 'single', this.money);
      }
    },
    /**
    ...
    select_money() {
      this.view_ctr_obj.is_empty_money = false;
      let input = this.$refs[`input-${this.id}`];
      if(input && this.vx_get_bet_single_list.length == 1) {
        // 设置选中
        input.$el.select();
        input = null;
        this.set_bet_obj_value("money", this.money);
...
     */
  ...
    },
    ...
    add_handle(type, index=1) {
      ...
      if(type =='ball_head') {      
        ...
        // 每次加号都需要掉用预约限额接口更新最新赔率
        if (this.get_bet_pre_timer) {
          clearTimeout(this.get_bet_pre_timer);
          this.get_bet_pre_timer = null;
        }
        this.get_bet_pre_timer = setTimeout(() => {
          this.query_pre_bet_amount((code, betAmountInfo) => {
            if(200 == code) {
              this.search_odds_value_by_ball_head();
            }
    ...
    search_odds_value_by_ball_head() {
      ...
      if(play_mapping.MARKET_RANG_FLAG_LIST.includes(this.play_id)) {
        ...
        for(let i = 0; i < ml_len; i++) {
          ...
          //这里要调整下
          let dl_fillter;
          // 针对篮球让分客队无法直接通过marketValue匹配当前选择的盘口时候兼容处理
          list.marketList.forEach(m_item => {
            if (Array.isArray(m_item.marketOddsList)) {
              m_item.marketOddsList.forEach(item => {
                if (item.playOptions == this.vx_get_bet_appoint_obj.computed_appoint_ball_head && item.oddsType == _.get(list, 'currentMarket.marketOddsList[0].oddsType',-1)) {
                  dl_fillter = {
                    ...m_item
                  };
                }
          ...
          if (!dl_fillter) {
            dl_fillter = list.marketList.filter(item => item.marketValue == this.vx_get_bet_appoint_obj.computed_appoint_ball_head)[0]; 
          }
          ...
          // 针对PB接口返回marketOddsList列表的对象属性变为$ref问题兼容
          parr.forEach(item => {
            if (item.$ref) {
              filter_arr = [{
                ..._.get(list, 'currentMarket.marketOddsList[0]')
              }]
            }
          });
          //投注项id 预约需要筛选
    ...
    appoint_handle() {
      ...
      if(!this.money && !this.max_money) {
        this.$root.$emit(this.emit_cmd.EMIT_BTN_CHANGE, true) 
      }
    },
    /**
    ...
    cancel_operate() {
      ...
      /* if(cs && cs.odds_switch && cs.odds_switch.includes(odds_table[this.vx_get_cur_odd]) && this.cur_odd == 'HK') {
  ...
  mounted() {
    ...
    if(len > 0) {
      ...
      // 当切换小窗口或内嵌版投注栏金额要保留
      this.$nextTick(()=>{
        if (this.vx_get_bet_current_money_obj[this.id] && this.is_common_amount) {
          this.money = this.vx_get_bet_current_money_obj[this.id]
        } 
      })
