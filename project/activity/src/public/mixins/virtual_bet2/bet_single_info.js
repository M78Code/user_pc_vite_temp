...
export default {
  ...
  data() {
    return {
      // 菜单数据
      menu_data: window.$menu.menu_data,
      oid: "",
      money: null,  //输入金额
      win_money: "", // 可赢额
      min_money: "", // 最小金额
  ...
  created() {
    ...
    this.$root.$on(this.emit_cmd.EMIT_BET_SINGLE_CHECK_MONEY_CMD, this.check_money);
    this.$root.$on(this.emit_cmd.EMIT_BET_SINGLE_RESET_CMD, this.bet_single_reset);
    this.$root.$on(this.emit_cmd.EMIT_BET_SINGLE_MIN_MONEY, this.set_min_money);
    this.$root.$on(this.emit_cmd.EMIT_INIT_BET_LIST_ITEM_CMD, this.init_bet_list_item);
    if(this.vx_get_virtual_bet_list.length==1) {
      this.setup_single_info = _.get(this.vx_get_user, 'cvo.single');
      if(!_.isEmpty(this.setup_single_info) && _.isObject(this.setup_single_info)) {
  ...
  methods: {
    ...
    */
    ...
    /**
     * @description: 更新键盘是否置灰
     * @param {number} value 键盘值
     * @return {undefined} undefined
    */
...
     */
  ...
  mounted() {
    ...
    this.focus_handle();
    let user = this.vx_get_user;
    //有最大值且输入金额大于最大金额，输入金额设置成最大金额
    if(this.max_money && this.money > parseFloat(this.max_money)) {
      this.money = parseFloat(this.max_money);
    }
    let balance = parseFloat(user.balance) || 0.00;
    //输入金额大于用户自有金额且自有金额大于0，输入金额等于自有金额
