    ...
    vx_get_virtual_bet_s_list: {
      handler(new_) {
        ...
        if(new_ && new_.length>0) {
          ...
          clearTimeout(this.timer_obj['input_focus']);
          this.timer_obj['input_focus'] = setTimeout(() => {
            let bet_input = this.$refs['bet-mix-input-0'];
            if(bet_input) {
              bet_input.$el.focus();
              bet_input = null;
            }
  ...
  methods: {
   ...
   /**
    ...
    mix_toggle_handle() {
      ...
      //计算投注框高度
      this.$root.$emit(this.emit_cmd.EMIT_COMPUTED_BET_HEIGHT_CMD);
    },
    /**
    ...
    set_bet_max_input(item){
      ...
      if(item.orderMaxPay) {
        if(this.vx_get_virtual_bet_s_list.length==1) {
          ...
          } else if(balance <= parseFloat(item.orderMaxPay)) {
            // 最高可以输入账户金额的值
            input_max = balance;
          }
        } else {
