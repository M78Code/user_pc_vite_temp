...
export default {
  ...
  data() {
    ...
    return {
      ...
      /** 根据mmp 或是 mct 匹配比分*/
      state_convert_dict: Object.freeze({
        //【篮球】——根据mmp
        2: {
          // 上半场比分
          1: "S2",
  ...
  computed: {
    mmp_map() {
      return this.$root.$t("mmp") || "";
    },
    mmp_map_v2() {
      return this.$root.$t("mmp_v2") || "";
    },
    msc_map() {
      return this.$root.$t("msc") || "";
    }
  },
  ...
  methods: {
    ...
    score_switch_handle(match) {
      ...
      if (Array.isArray(match.msc) && match.ms!=110) {
        ...
        // 需显示的比分集
        let score_dict = this.score_dict[csid] || [];
        // 篮球
        if ([2].includes(csid)) {
          // 全场
          if (match.mle != 17) {
        ...
        // 赛事阶段对应的 比分 key
        let cur_score_key = _.get(this.state_convert_dict, `${csid}.${_key}`)
