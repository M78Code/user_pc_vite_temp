...
<template>
  <div
    ...
    `csid${match.csid}`,!coumput_lock_state()||(odds_state!=='seal'&&coumput_lock_state()) ? odds_state:'odds_state', $route.params.video_size != 1 && 'new-bg']"
    @click.stop="bet_click"
    :id="DOM_ID_SHOW && `${bet_source}-${ol_data_item.oid}`"
  >
    <div class="bet-inner yb-flex-center fit"
    :class="odds_state=='seal' && coumput_lock_state() ? 'seal_on' :''"
    >
      <!-- 盘口 -->
      ...
      <div :class="['odds yb-number-font',odds_lift]" @click.stop="bet_click">
          <div v-if="odds_state=='seal' " class="lock"  :class="coumput_lock_state() ? 'seal_lock' : ''"> </div>
        <div 
...
export default {
    ...
    methods:{
       ...
       */
      coumput_lock_state(){
        if([367,368,369,7,20,74,103,241,341,342,343,236,344].includes(_.get(this.play_data,'hpid')*1)){
          return true 
        }else{
          return false
        }
