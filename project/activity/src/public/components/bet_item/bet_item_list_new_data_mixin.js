...
const bet_item = {
  ...
  computed: {
    ...
    is_deep(){
      if (this.handicap_list&&['1','1001'].includes(this.ol_data.csid)) {
          if (this.handicap_list.length>=6) {
        return this.col_index>=3&&this.col_index<=5
      }else{
        return this.col_index>=2&&this.col_index<=4
      }
      }else{
  ...
  watch: {
    ...
    'ol_data.ov': {
      handler(cur, old) {
        ...
        // 红升绿降变化
        this.set_odds_lift(cur, old,this.ol_data);
      }
    },
      ...
      if (this.odds_state != "lock" && this.odds_state != "seal" && old && !this.is_odds_seal()) {
        ...
        if (_odds_lift && !this.odds_lift_show) {
          ...
          this.timer_obj['odds_lift'] = setTimeout(() => {
            ...
            const odds_lift_obj = {
              odds_lift_show:this.odds_lift_show,
              odds_lift:this.odds_lift,
              oid:ol_data.oid
            }
            this.$emit('oddsChange',odds_lift_obj)
          }, 3000);
        }
      }
      const odds_lift_obj = {
