  ...
  <div class="q-pb-md">
    ...
    <div class="lineup-bg  relative-position">
      ...
      <div class="lineup-bg-wrap" :class="{'basketball': match.csid=='2'}">
        <!-- 主队 -->
        <div class="home" v-if="($route.name === 'analysis_header') || ($route.name !== 'analysis_header' && tabIndex == 1)" :class="`lineup-${_.get(lineupData, 'home.homeFormation')}`">
        ...
        <!-- 客队 -->
        <div class="away" v-if="($route.name === 'analysis_header') || ($route.name !== 'analysis_header' && tabIndex == 2)" :class="`lineup-${_.get(lineupData, 'away.awayFormation')}`">
          <div class="lineup-item">
    ...
    <div class="lineup-wrap">
      ...
      <div class="lineup-item home" v-if="($route.name === 'analysis_header') || ($route.name !== 'analysis_header' && tabIndex == 1)">
        <div class="panel">
          <!-- 首发阵容 -->
          <div class="panel-title">{{$root.$t('analysis.starting_lineup')}} {{match.csid=='1'?_.get(lineupData, 'home.homeFormation'):''}}</div>
          <div class="d-table">
            <div class="d-table-h">
              <!-- 位置 -->
              <div class="d-table-td">{{$root.$t('analysis.position')}}</div>
              <!-- 姓名 -->
              <div class="d-table-td">{{$root.$t('analysis.name')}}</div>
              <!-- 号码 -->
              <div class="d-table-td">{{$root.$t('analysis.number')}}</div>
            </div>
            <div class="d-tabel-tr" v-for="(item,index) in _.get(lineupData,'home.up')" :key="index">
              <div class="d-table-td" :class="{'multiple-pos': _.get(lineupData,'home.up').filter(item => item.positionName.length > 4).length}">
        ...
        <div class="panel">
          <!-- 替补阵容 -->
          <div class="panel-title">{{$root.$t('analysis.bench_lineup')}} {{match.csid=='1'?_.get(lineupData, 'home.homeFormation'):''}}</div>
          <div class="d-table">
            <div class="d-table-h">
              <!-- 位置 -->
              <div class="d-table-td">{{$root.$t('analysis.position')}}</div>
            ...
            <div class="d-tabel-tr" v-for="(item,index) in _.get(lineupData,'home.down')" :key="index">
              <div class="d-table-td" :class="{'multiple-pos': _.get(lineupData,'home.down').filter(item => item.positionName.length > 4).length}">
                <img v-img="([item.thirdPlayerPicUrl])" alt="" class="logo">
                <span class="player-name ellipsis">{{item.positionName}}</span>
              </div>
              <div class="d-table-td">
      ...
      <div class="lineup-item away" v-if="($route.name === 'analysis_header') || ($route.name !== 'analysis_header' && tabIndex == 2)">
        <div class="panel">
          <!-- 首发阵容 -->
          <div class="panel-title">{{$root.$t('analysis.starting_lineup')}} {{match.csid=='1'?_.get(lineupData,'away.awayFormation'):''}}</div>
          <div class="d-table">
            <div class="d-table-h">
              <!-- 位置 -->
              <div class="d-table-td">{{$root.$t('analysis.position')}}</div>
            ...
            <div class="d-tabel-tr" v-for="(item,index) in _.get(lineupData,'away.up')" :key="index">
              <div class="d-table-td" :class="{'multiple-pos': _.get(lineupData,'away.up').filter(item => item.positionName.length > 4).length}">
                <img v-img="([item.thirdPlayerPicUrl])" alt="" class="logo">
                <span class="player-name ellipsis">{{item.positionName}}</span>
              </div>
              <div class="d-table-td">
        ...
        <div class="panel">
          <!-- 替补阵容 -->
          <div class="panel-title">{{$root.$t('analysis.bench_lineup')}} {{match.csid=='1'?_.get(lineupData,'away.awayFormation'):''}}</div>
          <div class="d-table">
            <div class="d-table-h">
              <!-- 位置 -->
              <div class="d-table-td">{{$root.$t('analysis.position')}}</div>
            ...
            <div class="d-tabel-tr" v-for="(item,index) in _.get(lineupData,'away.down')" :key="index">
              <div class="d-table-td" :class="{'multiple-pos': _.get(lineupData,'away.down').filter(item => item.positionName.length > 4).length}">
                <img v-img="([item.thirdPlayerPicUrl])" alt="" class="logo">
                <span class="player-name ellipsis">{{item.positionName}}</span>
              </div>
              <div class="d-table-td">
    ...
    get_lineup(){
      api_analysis.get_lineupList({matchInfoId: this.match.mid}).then(({data}) =>{
        if(data.code == 200){
          ...
          if (_.get(lineupData, 'home.up')) {
            lineupData.home.up = home_up
          }
          if (_.get(lineupData, 'away.up')) {
