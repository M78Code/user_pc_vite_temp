...
<template>
  <div class="tab-odds">
    <div class="odds q-pb-md">
      <div class="tab">
        <!-- 让球 -->
        <span :class="{'active':tabIndex==1}" @click="tabClick(1)">{{$root.$t('analysis.rangqiu')}}</span>
        <!-- 欧指 -->
        <span :class="{'active':tabIndex==2}" @click="tabClick(2)">{{$root.$t('analysis.European_Finger')}}</span>
        <!-- 大小 -->
        <span :class="{'active':tabIndex==3}" @click="tabClick(3)">{{$root.$t('analysis.size')}}</span>
      </div>

      <div class="panel">
        <!-- 大小  胜平负  让球 -->
        <div class="panel-title">{{tabIndex == 3 ? $root.$t('analysis.size') : tabIndex == 2 ? $root.$t('analysis.European_Finger') : $root.$t('analysis.rangqiu')}}</div>
        <div class="d-header d-tr  relative-position">
          <template v-if="tabIndex==1">
            <!-- 公司 -->
            <div class="d-td">{{$root.$t('analysis.company')}}</div>
            <!-- 主胜 -->
            <div class="d-td">{{$root.$t('analysis.Main_win')}}</div>
            <!-- 盘口 -->
            <div class="d-td">{{$root.$t('analysis.handicap')}}</div>
            <!-- 客胜 -->
            <div class="d-td">{{$root.$t('analysis.away_win')}}</div>
          </template>
          <template v-if="tabIndex==2">
            ...
            <div class="d-td">{{$root.$t('analysis.away_win')}}</div>
            <!-- 主胜率 -->
            <div class="d-td">{{$root.$t('analysis.Main_win_rate')}}</div>
            <!-- 客胜率 -->
            <div class="d-td">{{$root.$t('analysis.Customer_win_rate')}}</div>
            <!-- 返还率 -->
            <div class="d-td">{{$root.$t('analysis.Return_rate')}}</div>
          </template>
          <template v-if="tabIndex==3">
            <!-- 公司 -->
            <div class="d-td">{{$root.$t('analysis.company')}}</div>
            <!-- 大 -->
            <div class="d-td">{{$root.$t('analysis.big')}}</div>
            <!-- 盘口 -->
            <div class="d-td">{{$root.$t('analysis.handicap')}}</div>
            <!-- 小 -->
            <div class="d-td">{{$root.$t('analysis.small')}}</div>
          </template>
        </div>
        ...
        <div class="d-body d-tr" v-for="(item,index) in odds_data" :key="index">
          ...
          <div class="content">
            <div class="handicap-before">
              <!-- 初盘 -->
              <span class="d-td timer">{{$root.$t('analysis.Initial_offer')}}</span>
            ...
            <div class="handicap-now">
              <!-- 即时 -->
              <p class="d-td timer">{{$root.$t('analysis.immediate')}}</p>
