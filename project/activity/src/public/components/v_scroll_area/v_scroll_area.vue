<!--
 * @Author: Amor
 * @Date: 2020-09-07 11:42
 * @Description: 通屏垂直滚动
-->

<template>
  <div class="v-scroll-area  fit row relative-position">
    <!-- 内容区 -->
    <div class="content-wrap fit">
      <!-- 头 ------------------->
      <div class="scroll-header absolute zindex">
        <slot name="header" />
        <q-resize-observer
          v-if="observer_area == 1 || observer_area == 3"
          @resize="on_header_change"
          :debounce="0"
        />
      </div>
      <!-- 中 ------------------->
      <q-scroll-area
        :visible="true"
        ref="ref_v_scroll_area"
        :thumb-style="{ right: $utils.is_iframe ? '-8px' : '-10.5px'}"
        class="fit v-scrollarea"
      >
        <q-scroll-observer @scroll="on_scroll" />
        <div
          class="middle-content"
          :style="{
            paddingTop: header_height + 'px',
            paddingBottom: footer_height + 'px',
          }"
        >
          <div class="scroll-inner-wrap fit">
            <slot />
            <q-resize-observer
              v-if="observer_middle"
              @resize="on_scroll_change"
              :debounce="0"
            />
          </div>
        </div>
      </q-scroll-area>

      <!-- 尾 ------------------->
      <div class="scroll-footer" :style="footer_position">
        <slot name="footer" />
        <q-resize-observer
          v-if="observer_area == 2 || observer_area == 3"
          @resize="on_footer_change"
          :debounce="0"
        />
      </div>
    </div>
  </div>
</template>

<script>
import { mapGetters } from "vuex";

export default {
  name: "VScrollArea",

  props: {
    // 固定区域 1：头部 2：尾部 3：头和尾
    observer_area: Number,
    // 是否监听中间滚动变化
    observer_middle: {
      tye: Boolean,
      default: false,
    },
    position: String,
    page_type: String,
  },

  data() {
    return {
      header_height: 0,
      middle_height: 0,
      footer_height: 0,

      // 底部区域位置样式
      footer_position: {
        bottom: "0px",
      },
      // 定时器ID
      bet_item_position_timer:0
    };
  },

  computed: {
    ...mapGetters({
      vx_layout_list_size: "get_layout_list_size",
    }),
  },

  methods: {
    /**
     * @description 响应 头部尺寸变化
     * @param  {type} height  高度
     * @return {undefined} undefined
     */
    on_header_change({ height }) {
      this.header_height = height;
      if (["details", "virtual_details", "home", "video"].includes(this.$route.name)) {
        // 判断当前是哪一个详情页，根据页面来发送对应的头部高度
        // 通用赛事详情
        if (this.page_type == "main_details") {
          this.$root.$emit("match_details_header_height_main_details", height)
          // 右侧详情
        } else if (this.page_type == "right_details") {
          this.$root.$emit("match_details_header_height_right_details", height)
          // 虚拟详情
        } else if (this.page_type == "virtual_details") {
          this.$root.$emit("match_details_header_height_virtual_details", height)
        }
      }
      // 虚拟体育右侧头部视频区高度
      if (this.page_type == "virtual_right_list") {
        this.$root.$emit("virtual_right_list_header_height", height);
      }
    },

    /**
     * @description 响应 主内容区尺寸变化
     * @param  {type} height  高度
     * @return {undefined} undefined
     */
    on_scroll_change({ height }) {
      this.middle_height = height;
      this.set_footer_position();
    },

    /**
     * @description 响应 尾部尺寸变化
     * @param  {type} height  高度
     * @return {undefined} undefined
     */
    on_footer_change({ height }) {
      this.footer_height = height;
      this.set_footer_position();
    },

    /**
     * @description 设置 尾部位置
     * @return {undefined} undefined
     */
    set_footer_position() {
      // 监听滚动变化触发逻辑
      if (this.observer_middle) {
        let left_height = this.vx_layout_list_size.height;
        //mac上面
        let dis = 5;
        if(/macintosh|mac os x/i.test(navigator.userAgent)) {
          dis = 15;
        }
        // 滚动底部高度
        let scroll_footer_top = this.header_height + this.middle_height + dis;
        // 滚动高度
        let scroll_height = scroll_footer_top + this.footer_height;
        // 如果滚动高度大于左侧高度按照底部距离为0处理
        if (scroll_height > left_height) {
          this.footer_position = {
            bottom: "0px",
          };
        } else {
          // 否则设置距离顶部距离
          this.footer_position = {
            top: scroll_footer_top + "px",
            bottom: "auto",
          };
        }
        // 最后再次设置滚动位置
        this.set_scroll_position([scroll_footer_top, 0]);
      } else {
        this.footer_position = {
          bottom: "0px",
        };
      }
    },

    /**
    * @description: 监听滚动
    * @param {Object} position 滚动位置
    * @return {}
    */
   on_scroll(position){
    this.$emit("on_scroll", position)
   },
   // 设置滚动位置
   set_scroll_position(position) {
     this.$refs.ref_v_scroll_area.setScrollPosition(position[0], position[1])
   }
  },
  created() {
    // 添加设置滚动位置监听
    this.$root.$on("set_scroll_position", this.set_scroll_position)
  },
  mounted() {
    // 监听窗口发生变化解决点击游览器全屏非全屏变化导致的css样式兼容问题
    window.addEventListener('resize', ()=>{
      this.set_footer_position()
    })
  },
  destroyed() {
    // 组件销毁清除设置滚动位置监听
    this.$root.$off("set_scroll_position", this.set_scroll_position)
    // 移除窗口监听，优化组件性能
    window.removeEventListener('resize', ()=>{})
  }
};
</script>

<style lang="scss">
.v-scroll-area {
  .content-wrap {
    flex: 1;
    box-shadow: var(--qq--box-shawdow-color);
    border-radius: 0px 6px 6px 0px !important;
    .scroll-header {
      position: absolute; 
      left: 0;
      right: 0;
      // z-index: 100;
      // background:var(--qq--y0-bg-color8);
      padding-right: 14px;
      // background-color: var(--qq--theme-bg-appoint-order);  //头部样式不一样 UI应该统一颜色
      &.zindex{
        z-index: 100;
      }
    }
    .scroll-inner-wrap .loading-wrap {
      padding-top: 100px !important;
    }
    .scroll-footer {
      position: absolute;
      left: 0;
      right: 0;
      bottom: 0;
      width: 220px;
      z-index: 100;
    } 
  }
  .v-scrollarea .q-scrollarea__bar {
    display: none;
  }
  .q-scrollarea__content {
    width: 100%;
  }
}
</style>
