...
<template>
  <div class="wrap-table">
    <div class="table-header">
      ...
      <!-- 总局数 -->
      <div class="table-col">{{$root.$t('results.total_play_count')}}</div>
      <!-- 赛盘 -->
      <div class="table-col">{{$root.$t('results.winning_set')}}</div>
    </div>
        ...
        <div class="tbale-body">
          <template v-for="(item, index) in results_list">
            ...
            >
              ...
              <!-- 1 -->
              <div class="table-col">
                <div>{{_.get(item, "scoreResult.S23.home", item.matchStatus==1?'':'-')||'-'}}</div>
                <div>{{_.get(item, "scoreResult.S23.away", item.matchStatus==1?'':'-')||'-'}}</div>
              </div>
              <!-- 2 -->
              <div class="table-col">
                <div>{{_.get(item, "scoreResult.S39.home", item.matchStatus==1?'':'-')||'-'}}</div>
                <div>{{_.get(item, "scoreResult.S39.away", item.matchStatus==1?'':'-')||'-'}}</div>
              </div>
              <!-- 3 -->
              <div class="table-col">
                <div>{{_.get(item, "scoreResult.S55.home", item.matchStatus==1?'':'-')||'-'}}</div>
                <div>{{_.get(item, "scoreResult.S55.away", item.matchStatus==1?'':'-')||'-'}}</div>
              </div>
              <!-- 4 -->
              <div class="table-col">
                <div>{{_.get(item, "scoreResult.S71.home", item.matchStatus==1?'':'-')||'-'}}</div>
                <div>{{_.get(item, "scoreResult.S71.away", item.matchStatus==1?'':'-')||'-'}}</div>
              </div>
              <!-- 5 -->
              <div class="table-col">
                <div>{{_.get(item, "scoreResult.S87.home", item.matchStatus==1?'':'-')||'-'}}</div>
                <div>{{_.get(item, "scoreResult.S87.away", item.matchStatus==1?'':'-')||'-'}}</div>
              </div>
              <!-- 总局数 -->
    ...
    results_list:{
      handler(res){
        ...
        res.forEach(item => {
          ...
          if(item.scoreResult != ""){
            obj.home = parseInt(_.get(item, "scoreResult.S23.home")||0) + 
                        parseInt(_.get(item, "scoreResult.S39.home")||0) + 
                        parseInt(_.get(item, "scoreResult.S55.home")||0) +
                        parseInt(_.get(item, "scoreResult.S71.home")||0) +
                        parseInt(_.get(item, "scoreResult.S87.home")||0)

            obj.away = parseInt(_.get(item, "scoreResult.S23.away")||0) + 
                        parseInt(_.get(item, "scoreResult.S39.away")||0) + 
                        parseInt(_.get(item, "scoreResult.S55.away")||0) +
                        parseInt(_.get(item, "scoreResult.S71.away")||0) +
                        parseInt(_.get(item, "scoreResult.S87.away")||0)
          }
