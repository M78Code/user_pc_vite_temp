...
<template>
  <div class="wrap-table">
    <div class="table-header">
      ...
      <!-- 总分 -->
      <div class="table-col">{{$root.$t('results.front_score')}}</div>
      <!-- 全场比分 -->
      <div class="table-col">{{$root.$t('results.all_score')}}</div>
      <!-- 安打比分 -->
      <div class="table-col">{{$root.$t('results.hit')}}</div>
    </div>
        ...
        <div class="tbale-body">
          <template v-for="(item, index) in results_list">
            ...
            >
              ...
              <div class="table-col">
                <div>
                  <div>{{_.get(item, "scoreResult.S127.home", item.matchStatus==1?'':'-')}}</div>
                  <div>{{_.get(item, "scoreResult.S127.away", item.matchStatus==1?'':'-')}}</div>
                </div>
              </div>
              <!-- 9 -->
              <div class="table-col">
                <div>
                  <div>{{_.get(item, "scoreResult.S128.home", item.matchStatus==1?'':'-')}}</div>
                  <div>{{_.get(item, "scoreResult.S128.away", item.matchStatus==1?'':'-')}}</div>
                </div>
              </div>
              <!-- 加时 -->
              <div class="table-col">
                <!-- 加时比分为0时不展示 -->
                <div v-if="_.get(item, 'scoreResult.S7.home') == 0 && _.get(item, 'scoreResult.S7.away') == 0">
                  <div></div>
                  <div></div>
                </div>
                <div v-else>
              ...
              <div class="table-col">
                <div>
                  <div>{{_.get(item, "scoreResult.S3015.home", item.matchStatus==1?'':'-')}}</div>
                  <div>{{_.get(item, "scoreResult.S3015.away", item.matchStatus==1?'':'-')}}</div>
                </div>
              </div>
            </div>
            <!-- 赛果详情 -->
