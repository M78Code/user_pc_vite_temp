        ...
        <div class="tbale-body">
          <template v-for="(item, index) in results_list">
            ...
            >
              ...
              <!-- 联赛 -->
              <div class="table-col">
                <!-- <img v-img="[_.get(item,'iconUrl')]" class="playback-logo" alt="" v-if="item.playBack"> -->
                <!-- 如果返回精彩回放就显示这个icon -->
                <div class="playback-logo" v-if="item.playBack&&show_play_back"></div>
                <img
                  v-img="[_.get(item, 'iconUrl')]"
              ...
              <div class="table-col">
                ...
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S14.home", "")
                        : _.get(item, "scoreResult.S14.home", "-")
                    }}
                  </div>
                  <div>{{ _.get(item, "scoreResult.S14.away", "-") }}</div>
                </div>
              </div>
              <div class="table-col">
                <!-- 黄牌下 -->
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S1402.home", "")
                        : _.get(item, "scoreResult.S1402.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S1402.away", "")
                        : _.get(item, "scoreResult.S1402.away", "-")
                    }}
                  </div>
                </div>
              ...
              <div class="table-col">
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S12.home", "")
                        : _.get(item, "scoreResult.S12.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S12.away", "")
                        : _.get(item, "scoreResult.S12.away", "-")
                    }}
                  </div>
                </div>
              ...
              <div class="table-col">
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S13.home", "")
                        : _.get(item, "scoreResult.S13.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S13.away", "")
                        : _.get(item, "scoreResult.S13.away", "-")
                    }}
                  </div>
                </div>
              ...
              <div class="table-col">
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S1302.home", "")
                        : _.get(item, "scoreResult.S1302.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S1302.away", "")
                        : _.get(item, "scoreResult.S1302.away", "-")
                    }}
                  </div>
                </div>
              ...
              <div class="table-col">
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S11.home", "")
                        : _.get(item, "scoreResult.S11.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S11.away", "")
                        : _.get(item, "scoreResult.S11.away", "-")
                    }}
                  </div>
                </div>
              ...
              <div class="table-col">
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S15.home", "")
                        : _.get(item, "scoreResult.S15.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S15.away", "")
                        : _.get(item, "scoreResult.S15.away", "-")
                    }}
                  </div>
                </div>
              ...
              <div class="table-col">
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S16.home", "")
                        : _.get(item, "scoreResult.S16.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S16.away", "")
                        : _.get(item, "scoreResult.S16.away", "-")
                    }}
                  </div>
                </div>
              ...
              <div class="table-col">
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S555.home", "")
                        : _.get(item, "scoreResult.S555.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S555.away", "")
                        : _.get(item, "scoreResult.S555.away", "-")
                    }}
                  </div>
                </div>
              ...
              <div class="table-col">
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S2.home", "")
                        : _.get(item, "scoreResult.S2.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S2.away", "")
                        : _.get(item, "scoreResult.S2.away", "-")
                    }}
                  </div>
                </div>
              ...
              <div class="table-col">
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S3.home", "")
                        : _.get(item, "scoreResult.S3.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S3.away", "")
                        : _.get(item, "scoreResult.S3.away", "-")
                    }}
                  </div>
                </div>
              ...
              <div class="table-col">
                <div class="color-highlight">
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S1.home", "")
                        : _.get(item, "scoreResult.S1.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S1.away", "")
                        : _.get(item, "scoreResult.S1.away", "-")
                    }}
                  </div>
                </div>
              ...
              <div class="table-col">
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S7.home", "")
                        : _.get(item, "scoreResult.S7.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S7.away", "")
                        : _.get(item, "scoreResult.S7.away", "-")
                    }}
                  </div>
                </div>
              ...
              <div class="table-col">
                <div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S170.home", "")
                        : _.get(item, "scoreResult.S170.home", "-")
                    }}
                  </div>
                  <div>
                    {{
                      item.matchStatus == 1
                        ? _.get(item, "scoreResult.S170.away", "")
                        : _.get(item, "scoreResult.S170.away", "-")
                    }}
                  </div>
                </div>
            ...
            <div
              class="wrap-load-box"
              :class="{
                playBack: item.playBack&&show_play_back
              }"
            ...
            >
              ...
              >
                ...
                >
                  <slider-x
                    ref="drag_scroll"
                    v-if="item.playBack && show_code_name_arr.length"
                  >
                    <template v-for="(slotProps,index) in show_code_name_arr">
                      <div class="video-history-item" :key="index" @click="handle_item_click(slotProps)">
                      ...
                      >
                        ...
                        <div class="tr-detail-item">
                          <div
                            ...
                            :key="j"
                          >
                            <span class="sizecolor" v-html="number_color(list2.playOptionName)"></span>
...
import Tabs from "../component/playback_tabs.vue";
import SliderX from "../component/playback_slider.vue";
...
export default {
  ...
  computed:{
    ...
    show_play_back(){
      return !!(this.vx_get_user.merchantEventSwitchVO&&this.vx_get_user.merchantEventSwitchVO.eventSwitch)
    },
    show_code_name_arr() {
      const type = this.current_events_type2 || 'all'
      const arr = this.results_playback_list || []
      if(type === 'all')return arr;
      if(type === 'goal')return arr.filter(v => v.eventCode === 'goal');
      if(type === 'corner')return arr.filter(v => v.eventCode === 'corner');
      return arr.filter(v => !['corner', 'goal'].includes(v.eventCode));
    },
  },
  data() {
    return {
  ...
  watch: {
    ...
    'vx_get_user.merchantEventSwitchVO':{
        handler(res) {
          // tab按钮开关
          let _tab_list = [
            {title: this.$root.$t('replay_video.all'), code: '0', type: 'all'},
            {title:this.$root.$t('replay_video.goal'), code: '1', type: 'goal'},
          ]
          if (res.cornerEvent) {
            _tab_list.push({title:this.$root.$t('replay_video.corner_kick'), code: '2', type: 'corner'})
          }
          if (res.penaltyEvent) {
            _tab_list.push({title:this.$root.$t('replay_video.punish'), code: '3', type: 'faqiu'})
          }
          // 当角球和罚牌都没有时不显示tab
...
.wrap-load-box{
  background-color: var(--qq--table_tr_detail);
