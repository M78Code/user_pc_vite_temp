  ...
  watch:{
    results_list:{
      handler(res){
        ...
        //累加局比分
        res.forEach(item => {
          let scores = item.scoreResult
          if(Object.keys(scores).length){
            let arr = [],
                home = 0,
                away = 0
            for(var i in scores) {
              let num_item = parseInt(i.replace("S",""))
              if(num_item >= 120 && num_item < 160){
                scores[i].home && (home += parseInt(scores[i].home))
                scores[i].away && (away += parseInt(scores[i].away))
                arr.push(scores[i])
              }
            } 
            item.score_total = {
