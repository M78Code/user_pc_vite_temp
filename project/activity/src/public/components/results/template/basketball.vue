...
<template>
  <div class="wrap-table">
    <div class="table-header">
      ...
      <!-- 上半场 -->
      <div class="table-col">{{$root.$t('results.first_half')}}</div>
      <div class="table-col">Q3</div>
      <div class="table-col">Q4</div>
      <!-- 下半场 -->
      <div class="table-col">{{$root.$t('results.second_half')}}</div>
      <!-- 加时 -->
        ...
        <div class="tbale-body">
          <template v-for="(item, index) in results_list">
            ...
            >
              ...
              <!-- 篮球3X3时只显示总分 -->
              <template v-if="_.get(item,'mle')==73">
              ...
              <template v-else>
                ...
                <div class="table-col">
                  <div>
                    <div class="home">{{_.get(item, "scoreResult.S22.home", item.matchStatus==1?'':'-')}}</div>
                    <div class="away">{{_.get(item, "scoreResult.S22.away", item.matchStatus==1?'':'-')}}</div>
                  </div>
                </div>
                <!-- 下半场 -->
                <div class="table-col">
                  <div>
                    <div>{{_.get(item, "scoreResult.S3.home", item.matchStatus==1?'':'-')}}</div>
                    <div>{{_.get(item, "scoreResult.S3.away", item.matchStatus==1?'':'-')}}</div>
                  </div>
                </div>
                <!-- 加时 -->
                <div class="table-col">
