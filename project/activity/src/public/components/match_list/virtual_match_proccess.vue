  ...
  <div  class="c-match-process virtual-sport">
    <div class="match-no-wrap inner-wrap row justfiy-start yb-flex-nowrap">
      <div  class="timer-wrap row items-center col-left">
        <!-- 倒计时图标 -->
        <icon v-show="cur_ountdown_second >= -1" name="icon-timer" size="14px" color="#F5F8FA" />
          <!-- 倒计时 -->
        <timer v-show="cur_ountdown_second >= -1"  :tconfig="{ time: Number(match.mgt), step: -1000, on_time_change, }" />
      </div>
      <!-- 当轮数 -->
      <div v-if="show_no" class="match_no ellipsis" v-tooltip="{content:match.no,overflow:1}">{{ substrName(match.no,match.csid) }}</div>
    </div>
  </div>
</template>
 
<script>
import { api_virtual } from "project/activity/src/public/api/index.js";//虚拟体育api
...
export default {
  ...
  created() {
    //设置各阶段的逻辑
    this.set_match_stage(this.match.mgt);
  },
  ...
  methods: {
     ...
     */
    on_time_change(obj) {
      this.set_match_stage(obj.tconfig.time, obj);
    },
     ...
     * @return {undefined} undefined
     */
    set_match_stage(millisecond, obj) {
      ...
      // 计时器调用
      obj && (obj.time_str = this.format_second_ms(second, "default"));

      /**最后 10 秒前调用————仅详情 */
      if(second > 10 && !this.can_bet_callback_once){
        this.can_bet_callback && this.can_bet_callback();
        this.can_bet_callback_once = true;
        //console.error("can_bet_callback")
      }

      /**【标题栏变成橙色】最后 60 秒————仅列表 **/
      if (second >= 0 && second <= 60 && !this.second60_callback_once) {
        this.second60_callback && this.second60_callback();
        this.second60_callback_once = true;
       // console.error("second60");
      }

      /**【停止投注】最后 10 秒 **/
      if (second >= 0 && second <= 10) {
        this.stop_bet_callback && this.stop_bet_callback(second);
        this.begin_callback_once = false;
        // console.error("stop");
        // 赛事结束禁止投注
      ...
      if (second <= 0) {
        ...
        // 篮球 
         if(match.csid == '1004'){
          ...
          // 已开赛 ————— 折叠赛事盘口
          if (!this.begin_callback_once) {
            this.begin_callback && this.begin_callback();

            this.begin_callback_once = true;
            // console.error("begin");
          }

          // （赛前 && 10 秒后） || （滚球 && 15 秒后）————为了拉最新的赛前
          if( (mmp == 'PREGAME' && second<=-10) || (mmp == 'INGAME' && second<=-15)){
            // 从未调用过
            if (!this.second15_fetch_list) {
              this.end_callback && this.end_callback();

              this.second15_fetch_list = true;

              // console.error("end");
            }
          }

          if(mmp == 'INGAME' && this.second15_fetch_list){
            // 未拉取到最大时长 每隔一秒拉一次
            if (!this.video_maxtime) {
              this.set_video_maxtime_lock || this.set_video_maxtime(second);
              //console.warn('max' ,this.second15_fetch_list,mmp)

            } else {
              this.set_callback(second);
            }
          }
      ...
      this.cur_ountdown_second = second;
    },
     ...
     * @return {undefined} undefined
     */
    set_video_maxtime(second) {
      this.set_video_maxtime_lock = true;
      let params = {
        tid: this.match.tid,
      ...
      api_virtual.get_video_maxtime(params).then((res) => {
        ...
        if (code == 200) {
          ...
          this.set_video_maxtime_lock = false;

          let video_maxtime = _.get(data, this.match.tid);

          if (video_maxtime) {
            this.video_maxtime = 1;
            //console.error(this.stay_time, video_maxtime);

            this.stay_time += -Number(video_maxtime);
          } else {
            this.set_callback(second);
          }
        }
