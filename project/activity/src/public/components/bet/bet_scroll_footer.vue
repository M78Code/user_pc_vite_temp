...
<template>
  <div class="bet-menu-wrap" :class="{'bet-menu-wrap-mix':!vx_is_bet_single}">
    ...
    <template v-if="bet_this">
      <div class="bottom-wrap bottom-wrap-a3" :class="{'bottom-cofirm':(bet_this.view_ctr_obj.order_confirm_complete>0)}">
        <div class="remove-invalid-item cursor-pointer text-center"  @click.stop="bet_this.remove_invalid_item" v-if="bet_this.show_invalid_btn">
        ...
        <template v-if="vx_is_bet_single">
          ...
          <!--这个错误提示信息一般预约状态时不显示除了0400540和0400483这个两个错误码-->
            <div class="bet-message text-center" v-if="!lock_btn || (this.vx_get_bet_appoint_obj && (bet_this.view_ctr_obj.error_code == '0400483' || bet_this.view_ctr_obj.error_code == '0400540' || bet_this.view_ctr_obj.error_code == 'M400004') )">
            ...
            <div class="full-width">
              ...
              <div class="row bet-buttons">
                ...
                >
                  <template v-if="['0400459','0400475','0400486','0400517','0400519','0400540'].includes(bet_this.view_ctr_obj.error_code)">
                    <!--确定按钮-->
                    {{$root.$t('common.confirm')}}
                  </template>
                  <template v-else>
                ...
                <div
                  ...
                  :id="DOM_ID_SHOW && `but-bet-single-submit`"
                  @click.stop="bet_this.go_history"
                  v-else-if="!bet_this.bet_confirm_show && ['0400483'].includes(bet_this.view_ctr_obj.error_code)"
                >
                ...
                <div
                  class="full-width cursor-pointer bet-delete-all bet-cancel-1"
            ...
            <div class="full-width bet-foot-menu">
              ...
              <div class="row check-box cursor-pointer" :class="{'checked': bet_this.is_common_amount}" @click.stop="bet_this.toggle_amount" v-if="bet_this.vx_get_bet_single_list.length==1">
          ...
          <template v-else>
            ...
            <div class="full-width bet-foot-save">
              <!--保留这些选项-->
              <div
                class="full-width cursor-pointer bet-save-items bet-cancel-1"
            ...
            <div class="full-width pt15">
              ...
              <div class="row bet-total">
                ...
                <div class="col-auto bet-total-right bet-gold-text">
                  {{bet_this.bet_total_win_money|four_five_six_double(2)}}
                </div>
              </div>
              <q-separator class="bet-mix-separator"></q-separator>
              ...
              <div class="row bet-buttons">
                ...
                <div class="full-width" v-else-if="!bet_this.bet_confirm_show  && !['0400483'].includes(bet_this.view_ctr_obj.error_code)">
...
export default {
  ...
  data() {
    return {
      version_name: window.env.config.pro_arr.FINAL_TARGET_PROJECT_NAME,  //版本号
      lock_btn: false     //是否失效
    };
  },
  mounted(){
    //按钮失效
    this.$root.$on(this.emit_cmd.EMIT_BTN_CHANGE, this.set_lock_btn)
  },
  created () {
     // 首次进入系统如果有弹窗应关闭弹窗 user_is_handle 判断用户首次进入页面
     this.$nextTick(()=>{
      if (!this.user_is_handle&&this.bet_this) {
      this.bet_this.cancel_handle()
    }
    })
    // 显示部分dom ID
  ...
  destroyed() {
    //按钮失效
    this.$root.$off(this.emit_cmd.EMIT_BTN_CHANGE, this.set_lock_btn)
  }
};
</script>
...
.bet-menu-wrap{
  ...
  background: var(--qq--theme-bg-record);
  .bottom-wrap.bottom-wrap-a3{
    background:transparent;
    // background: var(--qq--theme-bg-record);
  }
  .check-wrap{
...
.bet-cancel-1{
  // background: var(--qq--y0-bg-color2_1);
  background: var(--qq--theme-bet-delete-all-button);
  border: 0.5px solid var(--qq--y0-bg-color2_1_1) !important;
}
.bet-foot-menu{
  color: var(--qq--theme-color-bet-zone-right);
}
</style>