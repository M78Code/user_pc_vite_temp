    ...
    <q-card-section class="bet-item">
      <div class="row first-row no-wrap" v-if="match_type != 3">
        ...
        <div class="col-auto bet-icon-info">
          ...
          <template v-if="single_record_obj.orderStatusCode == 2">
            <!--投注确认中转圈，滚球才有的转圈圈-->
            <span class="bet-confirm-handle"><img
      ...
      <div class="bet-content">
        <div class="row">
          ...
          <label class="bet-play-text"><template v-if="match_type === 2"><label
                class="bet-match-playing">[{{ $root.$t('menu.match_playing') }}]</label></template>{{ single_record_obj.playName }}
            <template v-if="single_record_obj.scoreBenchmark
              ...
              ({{ single_record_obj?single_record_obj.scoreBenchmark.replace(':', '-'):''}})
            </template>
            <!--盘口类型-->
        ...
        <div class="row">
          <!--赔率-->
          <div class="col-auto yb-fontsize16 bet-odds-value" :class="{
            'up-red': odds_change_up,
            'down-green': odds_change_down
          }"><span class="odds-value yb-number-bold"><span>@</span>{{ single_record_obj.oddsValues |
  format_odds }}</span></div>
        </div>
      </div>

      <div class="row bet-win yb-fontsize12">
        <div class="col"
          v-if="single_record_obj.preOrderDetailStatus == 0 || single_record_obj.preOrderDetailStatus == 1">
          <!--投注额-->
