...
<template>
  <div class="c-match-date text-center" :class="{column:rows === 2}">
    ...
    <template
      v-if="([3,4].includes(Number(match_props.match.mcg)) || before_timeout || $utils.is_eports_csid(match_props.match.csid))  && !$utils.get_match_status(match_props.match.ms)"
    >
      <template>
        <span style="min-width: 60px">{{computed_today_early_date[0]}}<br/></span>
    ...
    <!-- 滚球(ms:1) -->
    <template v-if="$utils.get_match_status(match_props.match.ms,[110])">
      <!-- 计时器  -->
      <timer
      ...
      <!-- 显示:00:00 || 节制  -->
      <span v-show="[0,2].includes(inplay_match_type) ">{{$utils.counting_time_ctr_show_format(match_props.match,inplay_match_content)}}</span>
    </template>
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    let mstrc = _.get(this.match_props.match,'mstrc');
    if(_.get(this.match_props.match,'cds')=='C01' && mstrc){
  ...
  computed: {
    ...
    computed_today_early_date() {
      let _mgt = this.$utils.format_time_zone_millisecond(Number(this.match_props.match.mgt))
      var date_obj = this.format_date_base_obj(_mgt);
      let rs_date = this.$root.$t('time.time_date_5').replace('mm',date_obj.m).replace('dd',date_obj.d).replace('hh',date_obj.h).replace('ii',date_obj.mm)
      ...
      switch (csid) {
        ...
        case 11:
          ...
          if(mmps.includes(mmp))
          {
            ...
            if(_.get(this.match_props.match,'cds')=='C01' && csid==1){
              switch (_.get(this.match_props.match,'mle')+'') {
  ...
  methods: {
    ...
    cur_timer() {
      ...
      if(this.match_props.match.cur_timer){
          return timer +  Math.round((new Date().getTime() - this.match_props.match.cur_timer)/1000)
      }else{
        return timer
      }
    },
    ...
    inplay_timer_change(obj) {
     ...
     let {csid,tab_play_keys = "",tpl_id,mid} =  this.match_props.match || {}
      // 水球（16）显示为  < X分钟
      if(csid == 16){
        let mins =  _cur_time/60
        obj.time_str = this.$root.$t("match_info.match_date_format").replace('%s',mins)
      }
      // 足球计算15分钟玩法阶段 根据倒计时计算15分钟玩法阶段 避免时间改变赛事阶段切换延迟
      if ((tab_play_keys.includes('Minutes') || tpl_id == 24) && this.match_list_data) {
        let mst = parseInt(obj.tconfig.time) + obj.timer_tmp
        this.match_list_data.set_min15(this.match_props.match, mst, () => {
          this.$root.$emit(this.emit_cmd.EMIT_API_BYMIDS, {mids:[mid]});
        })
      }
    },
    ...
    before_start_timer_change(obj) {
      ...
      if (time_tmp < 60) {
        //一分钟之内
        obj.time_str = 1 + this.$root.$t("list.after_time_start2");
      } else {
        //大于一分钟的提示
        obj.time_str =
          Math.floor(parseInt(time_tmp / 60)) + ' '+
          this.$root.$t("list.after_time_start2");
      }
    },
    ...
    set_ininplay_match(mmp) {
      ...
      if (this.$utils.get_match_status(match.ms)) {
        if ([1,11,14].includes(csid)) {
          ...
          let not_show =( csid==2 && mmp==31) || (csid!=4 && mmp===32) ||(csid===6 && match.mst == 0 && mmp==40) ||([4,15,16].includes(csid) && [34,50].includes(mmp))
          ...
          } else if (mmp == 100 ||  match.mst == 0 || (csid==4 &&[32].includes(mmp))) {
            this.inplay_match_type = 0;
            this.inplay_match_content = "00:00";
          } else {
            this.inplay_match_type = 1;
