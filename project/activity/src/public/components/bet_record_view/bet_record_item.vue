      ...
      <template v-if="order.matchType!=3">
        <div class="row">
          <div class="col bet-against">
            <!--主队 v 客队-->
            <span class="home-vs-away">
              {{home}}<span class='bet-pk'>v</span>{{(away.indexOf('(') > -1) ?(away.substr(0, away.lastIndexOf('('))):away}}
              <!--有括号就是有比分信息 当鼠标移上去显示，移出去就消失-->
              <span v-if="away.indexOf('(') > -1" 
              @mouseover="show_score_info=true"
              @mouseout="show_score_info=false"
              >{{away.substr(away.lastIndexOf('('))}}</span>
              <!--对阵信息后面加的那个提示-->
              <q-tooltip
        ...
        <div class="col bet-play-game yb-fontsize13">
          ...
          <template v-else> 
            <label class="bet-play-handicap">
              ...
              <span class="sport-type">{{match_type(order.matchType,lang_code)}}{{order.playName}}</span>
...
.sport-type{
  color: var(--qq--match-new-text-color2);
}
</style>
