...
<template>
  <div class="tab-wrap fit  relative-position" ref="wrap" @mousedown="mousedown">
    ...
    <div class="item-wrap  relative-position" ref="item-wrap" :style="{left:item_wrap_left+'px'}" :key="key">
      ...
      <template v-if="currentIndex!==-1 && width > 0 && is_show_line && $route.params.video_size != '1'">
    ...
    <template v-if="is_show_btn">
      <div class="left-btn btn" style="border-top:0px" v-show="left_btn_show && !is_drag" @click="item_wrap_left += 100"><div class="yb-icon-arrow"></div></div>
      <div class="right-btn btn" style="border-bottom:0px" v-show="right_btn_show&& !is_drag" @click="item_wrap_left -= 100"><div class="yb-icon-arrow"></div></div>
    </template>
  </div>
</template>
