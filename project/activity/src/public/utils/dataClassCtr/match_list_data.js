...
export default class MatchListDataClass {
  ...
  */
  constructor() {
    // 球种赛事数量
    this.sport_match_count = _.cloneDeep(sport_match_count_template)
  }
  /**
   * @Description 初始化
   * @param {undefined} undefined
  ...
  */
  compute_match_list_all_data(match_list,is_ws_call,is_all_match){
    ...
    // 球种赛事数量
    let sport_match_count = _.cloneDeep(sport_match_count_template)

    // 根据商户过滤篮球赛事
    // if(store.getters.get_user.mId == '1443742662615240704'){
    //   _.remove(match_list, match => match.csid == 2)
    // }
    match_list.forEach(match => {
      ...
      if (is_all_match && !utils.is_virtual_csid(csid)) {
          // 统计球种赛事数量
          sport_match_count['csid_' + csid] = sport_match_count['csid_' + csid] || { count: 0 }
  ...
  */
  compute_match_data(match){
    ...
    if($menu.get_match_list_api_params().euid == 30199 && window.vue.$route.name !=='search'){
      match.is_hot = true
    }

    // 设置赛事logo
    match.match_logo = {
      home_1_logo:_.get(match,'mhlu[0]'),
      home_1_letter:_.get(match,'frmhn[0]'),
      home_2_logo:_.get(match,'mhlu[1]'),
      home_2_letter:_.get(match,'frmhn[1]'),
      away_1_logo:_.get(match,'malu[0]'),
      away_1_letter:_.get(match,'frman[0]'),
      away_2_logo:_.get(match,'malu[1]'),
      away_2_letter:_.get(match,'frman[1]'),
      is_double:_.get(match,'mhlu.length') > 1
    }
  ...
  set_match_all_handicap_data(match){
    ...
    add_hps.forEach( item => {
      ...
      hls_arr.forEach( (hl,hl_index) => {
        this.compute_hl_obj_data({match,hl_obj:{hl,hpid,chpid},all_hids,all_hl_obj,hl_index:hl_index+2})
      })
    })
  ...
  compute_match_all_handicap_data(match){
    ...
    // 网球准确局数 | 排球准确局数 | 羽毛球准确局数 根据赛制获取主盘列表
      ...
      if([5,9].includes(+csid)){
        main_handicap_list = this.clone_arr(play_config[`main_handicap_list_5_${mft}`])
      }else{
        main_handicap_list = this.clone_arr(play_config[`main_handicap_list_${csid}_${mft}`])
      }
    }
    // 兵乓球准确局数  根据赛制获取主盘列表
    else if(tpl_id == 15){
      ...
      main_handicap_list = this.clone_arr(play_config[`main_handicap_list_${mft}`])
    }
    // 斯诺克让球与大小主盘列表
    ...
    //  15分钟主盘口列表
      ...
      main_handicap_list = this.clone_arr(match_list_play_config.template_0.hps15Minutes)
      type = 4
      this.set_min15(match,match.mst)
    }
    //  罚牌主盘口列表
    else if(tpl_id == 25  && csid == 1){
      main_handicap_list = this.clone_arr(match_list_play_config.template_0.hpsPunish)
    }
    match.main_handicap_list = this.merge_template_data({match,handicap_list:main_handicap_list,type})
  ...
  */
  compute_match_all_handicap_data_champion(match){
    ...
    // 遍历主盘口数据
    _.each(match.hpsData, hpsData => {
      _.each(hpsData.hps, item => {
        let hl_obj = _.get(item,'hl',{})

        if(hl_obj.hid){
          hl_obj.end_time = this.time_conversion(hl_obj.hmed)
          hl_obj.hpn = _.get(match.play_obj,`hid_${hl_obj.hid}.hpn`,'')
  ...
  */
  set_match_cur_handicap_data(match,is_ws_call){
    ...
    cur_handicap_list =  this.merge_template_data({match,handicap_list:cur_handicap_list,type:1})
    // 是否ws调用
    if(is_ws_call){
      _.merge(match.cur_handicap_list,cur_handicap_list)
    }else{
      match.cur_handicap_list = cur_handicap_list
    }
    // 设置是否显示当前局玩法
    match.is_show_cur_handicap = match.tpl_id == 7 ? this.get_basketball_is_show_cur_handicap(match) : utils.get_match_status(match.ms,[110]) == 1
  }
  /**
  ...
  get_cur_handicap_list(match,play_config){
    ...
    if(match.tpl_id == 7){
      switch (+match.mmp) {
        ...
        case 31: //中场休息
        case 2: //下半场
          cur_handicap_list = this.clone_arr(play_config.cur_handicap_list_down)
          break;
        case 13: //第一节
          cur_handicap_list = this.clone_arr(play_config.cur_handicap_list_1)
          break;
        case 301: //第一节休息
          cur_handicap_list = this.clone_arr(play_config.cur_handicap_list_1_rest)
          break
        case 14: //第二节
  ...
  */
  set_match_add_handicap_data(match){
    ...
    match.add1_handicap_list = this.merge_template_data({match,handicap_list:add1_handicap_list,type:2})
    match.add2_handicap_list = this.merge_template_data({match,handicap_list:add2_handicap_list,type:3})
  }
  /**
  ...
  */
  compute_other_play_data(match){
    ...
    // 其他玩法盘口列表
    let other_handicap_list = this.clone_arr(match_list_play_config[template_name][cur_other_play])
    // 波胆
    ...
    // 组合玩法 特殊处理
    if(cur_other_play =='hpsCompose') {
      console.log('other_handicap_list', match_list_play_config[template_name][cur_other_play]);

      other_handicap_list = this.clone_arr(match_list_play_config[template_name][cur_other_play])
    }

    if(!cur_other_play){
      other_handicap_list = this.clone_arr(match_list_play_config[template_name].hpsCorner)
    }
    // 4：15分钟玩法 1：其他玩法
  ...
  */
  ...
  get_5minutes_template(match = {}) {
    ...
    let tpl_name = `hps5Minutes_${hpid}`
      ...
      _.each(match_list_play_config[`template_${tpl_id}`][tpl_name],col=>{
  ...
  merge_template_data({match,handicap_list,type,play_key}){
    ...
    handicap_list.forEach((col, col_index) => {
      col.ols.forEach((ol, ol_index) => {
        ...
        let { hn, _hpid, ot} =  ol
        if(type == 4){
          handicap_type =  this.get_min15_handicap_type(length,match.hSpecial,col_index)
  ...
  */
  switch_other_play(mid,play_key){
    ...
    if(play_key === 'hpsCompose'){
      other_handicap_list = this.clone_arr(match_list_play_config[template_name]['hpsCompose'])
      match = {}
    }
  ...
  */
  compute_ol_data(hl_data,match,handicap_type){
    ...
    _.each( hl_data.ol, ol => {
      ...
      ol.csid = match.csid
      //内嵌简化盘口文本
      if(utils.is_iframe && match.tpl_id != 18){
        // 组合玩法需要特殊处理 因为接口字段是从onbl拿的
        ol.onbl =  this.disk_text_replace(store.getters.get_lang, _.get(ol,'onbl', ''))
        ol.onb =  this.disk_text_replace(store.getters.get_lang, _.get(ol,'onb', ''))
      }
      //简化盘口文本
  ...
  */
  set_match_ol_obj(match_list){
    match_list.forEach( _match => {
      ...
      // 所有盘口数据的key
      let all_hl_key = ['main_handicap_list','add1_handicap_list','add2_handicap_list','cur_handicap_list','other_handicap_list']
      all_hl_key.forEach( hl_key => {
        // 遍历所有盘口数据
        _.each(match[hl_key], hl_data => {
          // 冠军玩法设置hl_obj对象
  ...
  */
  set_match_score(match){
    ...
    let score_obj = utils.serialized_score(_.get(match,'msc',[]),true)
  ...
  */
  ws_update_score(ws_data){
    let match = this.mid_obj['mid_'+_.get(ws_data,'cd.mid')]
    // 未开赛不用设置比分
    ...
    let score_obj = utils.serialized_score(_.get(ws_data,'cd.msc',[]))
    // 合并比分对象
    _.merge(match.score_obj,score_obj)
    // 设置主比分
    this.set_main_score(match)
    // 设置历史比分列表
    this.set_history_score_list(match)
  ...
  set_main_score(match) {
    ...
    Object.assign(match,{
      // 主队比分
      home_score: _.get(score_obj,`${key}.home`,'0'),
      // 客队比分
      away_score: _.get(score_obj,`${key}.away`,'0')
    })
  }
  /**
  ...
  set_history_score_list(match) {
    ...
    // 赛事阶段对应的 比分 key
    let cur_score_key = _.get(match_state_convert_score_dict, `csid_${csid}.mmp_${match.mmp}`)
    // 斯诺克根据第几局获取比分key
    if (csid == 7) {
      cur_score_key = _.get(match_state_convert_score_dict, `csid_${csid}.${match.mct}`)
    }
    ...
    if(match.csid== 2 && match.mmp == 14 && match.up_half_text){
      match.cur_score  = _.get(match,'score_obj.S2',{home:'',away:''})
    }else{
      match.cur_score = _.last(score_list) || {home:0,away:0}
    }
    match.score_list = score_list
    this.coverage_match_data({score_list},match.mid)
    // 设置总分
    match.total_score_str = `${total_home}-${total_away}(${total_home+total_away})`
  }
  /**
   * @Description 覆盖赛事数据
  ...
  */
  get_first_unfold_mids(){
    ...
    match_status_type_arr.forEach(match_status_type => {
      // 遍历联赛列表
      let league_list = _.get(this.league_list_obj,match_status_type,[])
  ...
  */
  ws_merge_match(ws_data,merge_keys){
    ...
    // 如果是ws推送的时间戳比接口更新时间小说明不是最新的推送数据不进行更新处理(数据源C01时,不做这样处理)
    if(ws_data.ctsp < match.api_update_time){
      if(!(match.cds=='C01' && _.get(ws_data,'cmd') == 'C110')){
        return merge_state;
      }
    }
    let data = {}
  ...
  */
   compute_sport_count(remove_mid){
    // 球种赛事数量
    let sport_match_count = _.cloneDeep(sport_match_count_template)
    let csid = _.get(this.mid_obj['mid_'+remove_mid],'csid')
    if(csid){
      sport_match_count['csid_'+csid] = {count:0}
    }
    this.match_list.forEach( match => {
      // 统计球种赛事数量
      sport_match_count['csid_'+match.csid] = sport_match_count['csid_'+match.csid] || {count:0}
    ...
    $menu.compute_play_all_menu_count(sport_match_count)
    // 该赛种 已全部移除
    if(sport_match_count['csid_'+csid] && sport_match_count['csid_'+csid].count === 0){
  ...
  */
  set_league_list_collect(tid,status,is_league_list ){
    if(is_league_list){
      ...
      _.each(match_status_types, type=> {
        // 遍历联赛列表
        let league_list = _.get(this.league_list_obj, type ,[])
        _.each(league_list, league => {
          if(league.tid === tid) {
            league.tf = status
            return
          }
      ...
      _.each(this.match_list,match_item=>{
