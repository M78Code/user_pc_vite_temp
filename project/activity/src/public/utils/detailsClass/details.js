  ...
  */
  sr_click_handle(match) {
    ...
    if([1,2].includes(match.csid*1)){
      let token = window.vue.$store.getters.get_user.token || sessionStorage.getItem("pc_token") || "";
      full_url = `/?token=${token}&gr=${window.env.config.gr}&userId_theme=${store.getters.get_theme}#/analysis_header/${match.csid}/${match.mid}` // seid,match.srid
