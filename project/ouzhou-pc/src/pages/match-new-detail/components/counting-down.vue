  ...
  <div
    ...
    :class="{
      'counting-down-wrap-no-timer':
        (!show_time_counting || counting_time === '00:00') && !home,
    }"
  >
    <span style="margin-right: 6px;color: #8a8986;font-size: 14px;" v-html="computed_process_name"></span>
    ...
    <span
      v-if="show_time_counting"
      ref="counting"
      class="counting"
      v-html="utils.counting_time_ctr_show_format(match, counting_time)"
    ></span>
    <span
      ref="special-match"
      class="special din-regular"
      :class="{ 'mar-l5': u_like }"
      :data-d="`${match.csid}-${match.mmp}`"
      v-show="
        [2, 4, 6, 15, 16].includes(+match.csid) &&
        [301, 302, 303].includes(+match.mmp)
      "
      ><!--csid:16水球-->
      {{ match.mlet }}
    </span>
...
import { i18n_t, utils, get_match_status } from "src/core/index.js";
...
const counting_wrapper_width = ref(0.8); // 定时器容器宽度
...
const show_time_counting = computed(() => {
  ...
  // 美式足球
    ...
    if (mmp === 40) {
      // 2843 【SIT】【H5】列表页，美足加时赛阶段，期望优化时间展示
...
const counting_down_wrap_width = (title, counting_time) => {
  ...
  // 特殊赛事是否显示时间，如水球
  const special_match_time_show =
    [2, 4, 6, 15, 16].includes(+props.match.csid) &&
    [301, 302, 303].includes(+props.match.mmp);

  // 中文、繁体定时器容器宽度适配
  if (["zh", "tw"].includes(lang)) {
    // 猜你喜欢
    if (props.u_like && title_length === 3) {
      counting_wrapper_width.value = show_time_counting.value
        ? 0.39 + 0.05 + 0.3
        : "auto";
    }
    // 上半场 xx:xx
      ...
      // 水球等赛事，宽度适配
      const time_width =
        counting_time.value && isNaN(counting_time.value.charAt(0))
          ? 0.55
          : 0.3;
      counting_wrapper_width.value = show_time_counting.value
        ? 0.39 + 0.05 + time_width
        : special_match_time_show
        ? 0.39 + 0.05 + 0.3
        : 0.39 + 0.05;
    }
    // 下半场 xxx:xx
    else if (title_length === 3 && time_length === 6) {
      counting_wrapper_width.value = 0.39 + 0.05 + 0.36;
    }
    // 中场休息
    ...
    // 其他
      ...
      counting_wrapper_width.value =
        ...
        (MenuData.get_menu_type() === 3000 && counting_time.value == "00:00")
          ? "auto"
          : (title_length > 2 ? 0.35 : 0.19) + 0.05 + 0.06 * (time_length || 5);
    }
  }
...
const counting_frame = () => {
  ...
  if (route.name === "home" && hotReqTime.value && props.match.mst !== "0") {
    ...
    reduce_second.value =
      props.match.mst * 1 + (props.is_add ? runningSeconds : -runningSeconds);
  }
...
const set_min15 = (mst, callback) => {
  ...
  if (mst < 900) {
    ...
    mmp_15_min = 2;
  } else if (mst < 2700 || [6, 41].includes(Number(props.match.mmp))) {
    // 中场加时判断
    mmp_15_min = 3;
  } else if (mst < 3600) {
    mmp_15_min = 4;
  ...
  if (
    props.match.mmp_15_min &&
    mmp_15_min !== props.match.mmp_15_min &&
    callback
  ) {
    callback(mmp_15_min);
  }
...
const set_min5 = (mst, callback) => {
  ...
  if (mst < 300) {
    ...
    mmp_5_min = 2;
  } else if (mst < 900 || [6, 41].includes(Number(props.match.mmp))) {
    // 中场加时判断
    mmp_5_min = 3;
  } else if (mst < 1200) {
    mmp_5_min = 4;
  ...
  // 如果默认阶段和接口数据返回的mmp_15_min不一致 重新调用接口获取
  if (
    props.match.mmp_5_min &&
    mmp_5_min !== props.match.mmp_5_min &&
    callback
  ) {
    callback(mmp_5_min);
  }
...
const get_counting_time = () => {
  ...
  if (props.match.csid == 16) {
    ...
    counting_time.value = counting_time.value.replace(
      "<",
      "<span style='font-family:-apple-system,Helvetica Neue,Helvetica,Arial,sans-serif'><</span>"
    );
  } else {
    counting_time.value = `${minutes.padStart(2, "0")}:${second.padStart(
      2,
      "0"
    )}`;
  }
  ...
  if (counting_time.value.startsWith("-")) {
    ...
    // 不足两位，加'0'处理
    counting_time.value =
      (counting_time.value.charAt(1) === ":" ? "0" : "") + counting_time.value;
  }
};
