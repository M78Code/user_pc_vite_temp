...
<template>
  ...
  function get_match_info(show_loading = true) {
    ...
    const { mid } = match_info;
    video.api_get_match_info(mid, $route, (match_info, _load_data_state) => {
      load_data_state.value = _load_data_state
      // match_info_ctr.init_match_obj(match_info); // 初始化赛事控制类
      // match_info = this.match_info_ctr.match_obj
    })
  }
  //设置全屏状态
