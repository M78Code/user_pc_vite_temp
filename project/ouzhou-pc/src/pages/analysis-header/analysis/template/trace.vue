 ...
 * @FilePath: /user-pc1/src/public/components/analysis/template/trace.vue
 * @Description: 足篮赛事分析--赛况--事件线
 * @Author:
-->
<template>
  <div>
    <div class="event-item  relative-position" v-for="(item,index) in event_data" :key="index">
      ...
      <div class="info">
        ...
        <template v-if="match.csid=='1'">
          <img :src="`/image/yabo/svg/analysis-${item.icon}.svg`" alt="" class="sign-icon" v-if="item.icon" width="14" style="vertical-align: middle">
        </template>
        <!-- 篮球 -->
        <template v-else>
