    ...
    <!-- 杯赛积分 -->
      <div v-if="vs_info&&vs_info.length>0" class="panel integrate" :class="{'show_all_rank': flag}">
        <div class="panel-title">
          <!-- 联赛积分(无数字) 杯赛积分(有数字) -->
          <span v-if="tournamentTypeFinish">{{i18n_t(`analysis.cup_points${lodash.get(vs_info,'[0].tournamentType')==2?'1':''}`)+'-'+match.tnjc}}</span>
          <!-- 查看更多 -->
          <span 
            class="ranking-more" 
        ...
        <!-- 展开收起按钮 -->
        <div class="row justify-center" v-if="match.csid == '1'&&vs_info_data.length>2">
          <div class="unfold-table" @click="unfoldTable">{{unfold?i18n_t('icon_tips.fold'):i18n_t('icon_tips.unfold')}}</div>
        </div>
      </div>
      ...
      <div class="panel future details" v-if="match.csid=='1'">
        ...
        <!-- 有内容的情况才渲染，方便调整样式 -->
        <div class="content" v-if="(lodash.get(baseData,'sThirdMatchFutureStatisticsDTOMap.1') && lodash.get(baseData,'sThirdMatchFutureStatisticsDTOMap.1').length) && (lodash.get(baseData,'sThirdMatchFutureStatisticsDTOMap.2') && lodash.get(baseData,'sThirdMatchFutureStatisticsDTOMap.2').length)">
          <div class="wrap-home">
            <div class="future-item" v-for="(item,index) in lodash.get(baseData,'sThirdMatchFutureStatisticsDTOMap.1')" :key="index">
          ...
          <div class="wrap-away">
            <div class="future-item" v-for="(item,index) in lodash.get(baseData,'sThirdMatchFutureStatisticsDTOMap.2')" :key="index">
              <div class="match-time ellipsis">
                <span>{{item.beginTime ? formatTime(item.beginTime, 'yy-mm-dd') : '-'}}</span>
                <!-- 天后 -->
                <span>{{item.intervalDay || '-'}} {{i18n_t("analysis.day_later")}}</span><br>
      ...
      <div class="panel hurt details" v-if="match.csid=='1'">
        ...
        <div class="simple-title " :class="{'simple-team':(lodash.get(baseData,'sThirdMatchSidelinedDTOMap.1') && lodash.get(baseData,'sThirdMatchSidelinedDTOMap.1').length) && (lodash.get(baseData,'sThirdMatchSidelinedDTOMap.2') && lodash.get(baseData,'sThirdMatchSidelinedDTOMap.2').length)}">
          <div class="home">
            <img v-img="([lodash.get(match,'mhlu[0]'),lodash.get(match,'frmhn[0]')])" class="logo" alt/>
            <span>{{match.mhn}}</span>
          </div>
        ...
        <!-- 有内容的情况才渲染，方便调整样式 -->
        <div class="content" v-if="(lodash.get(baseData,'sThirdMatchSidelinedDTOMap.1') && lodash.get(baseData,'sThirdMatchSidelinedDTOMap.1').length) && (lodash.get(baseData,'sThirdMatchSidelinedDTOMap.2') && lodash.get(baseData,'sThirdMatchSidelinedDTOMap.2').length)">
          <div class="wrap-home">
