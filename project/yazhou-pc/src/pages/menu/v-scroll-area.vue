  ...
  <div class="v-scroll-area  fit row relative-position">
    ...
    <div class="content-wrap fit">
      ...
      <q-scroll-area :visible="true" ref="ref_v_scroll_area"
        :thumb-style="{ right: utils.is_iframe ? '-8px' : '-10.5px' }" class="fit v-scrollarea">
...
const on_header_change = ({ height }) => {
  ...
  if (["details", "virtual_details", "home", "video"].includes('home')) {
...
const set_footer_position = () => {
  ...
  if (props.observer_middle) {
    let left_height = 800 //this.vx_layout_list_size.height;
    //mac上面
    let dis = 5;
...
const set_scroll_position = (position) => {
  currentInstance.appContext.$refs.ref_v_scroll_area.setScrollPosition(position[0], position[1])
}


onMounted(() => {
  // 添加设置滚动位置监听
  // this.$root.$on("set_scroll_position", set_scroll_position)
...
onUnmounted(() => {
  // 组件销毁清除设置滚动位置监听
  // this.$root.$off("set_scroll_position",set_scroll_position)
  // 移除窗口监听，优化组件性能
