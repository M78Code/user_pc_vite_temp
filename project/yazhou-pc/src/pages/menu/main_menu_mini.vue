    ...
    <v-scroll-area ref="ref_bet_scroll_area" position="menu">
      <div class="item-wrap column items-center yb-padding-t5">
        ...
        <div @click.stop="menu_item_click('bet_history')" class="menu-item">
          <img
            class="menu_mini-bet"
            src="~public/image/yabo/svg/menu_mini_bet.svg"
          />
        </div>
        <!-- 现场滚球 -->
        ...
        <div
          :key="`${item1}`"
          @click="lev_1_click_min(item1)"
          class="menu-item"
          v-if="!BaseData.filterSport_arr.includes(''+BaseData.compute_sport_id(item1))&& (item1 == 300 || lv_1_num(item1))"
        >
          <!-- menuType == 3000 电竞 -->
          <sport-icon :data-id="item1" :sport_id="BaseData.compute_sport_id(item1)" size="18px" class="icon" status="2" />
        </div>
        </template>
        <!-- 你菜单分割线 -->
        <div class="dividing-line"></div>
...
//   },
  methods: {
    ...
    new_menu_click(val) {
      ...
      this.NewMenu.set_left_menu_result({
        root:val,
        lv1_mi:'',
        lv2_mi,
        sports:'',
