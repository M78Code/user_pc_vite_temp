...
import { loadLanguageAsync, compute_css, useMittOn, MITT_TYPES, MenuData, UserCtr,MatchDataWarehouse_PC_Detail_Common } from "src/core/index.js";
...
export  const useRightDetails = ((props)=>{
  ...
  //    show_more(){
  //      // 当前在详情页或者电竞页的时候展示
  //      if((route.name=='details' || (this.is_esports && route.name != 'video')) &&  route.name !=='search'){
  //        return true
  //      } else{
  //        // 如果不在详情页，就在关盘的时候展示
  ...
  //        is_esports = this.$utils.is_eports_csid(route.params.csid)
  ...
  //      // 电竞不用请求玩法数据
  //      if((this.is_esports && route.name != 'video') || route.name == 'details'){
  //        return
  //      }
  ...
  //      // 如果当前是电竞页，就不请求右侧玩法列表
  //      if(this.is_esports && route.name != 'video'){
  //        return false
  //      }
  //      // 如果当前在详情页或者接收数据的 mid 和当前 mid 不一样也不展示玩法列表
  //      if(route.name == "details" || (obj.mid && this.mid != obj.mid)){
  //        return false
  //      }
  ...
  //      let  fun_temp =async ()=>{
  //          // 记录当前请求gcuuid
  //          this.last_by_mids_uuid = params.gcuuid = UserCtr.uid()
