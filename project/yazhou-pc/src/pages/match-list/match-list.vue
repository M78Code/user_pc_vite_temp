...
<template>
  <div>
      <match-list-card v-for="card_key in match_list_info" :key="card_key + match_list_info" :card_key="card_key" />
    </div>
...
export default {
  ...
  setup() {
    const match_list_info = ref(MatchListCardData.match_list_card_key_arr)
    setTimeout(() => {
      match_list_info.value = MatchListCardData.match_list_card_key_arr;
    }, 1000);
    onMounted(() => {
      mounted_fn()
    })
