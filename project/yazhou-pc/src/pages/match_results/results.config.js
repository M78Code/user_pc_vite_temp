 ...
 //移除相应监听事件 //视图销毁钩子函数内执行
  ...
  const input_focus=()=> {
    ...
    const showDate=(start, end)=> {
      let _start = start || state.format_day(state._date.getTime());
      let _end =
        end || state.format_day(state._date.getTime() + 1000 * 60 * 60 * 24);
      // 越南语时间格式和其他语言不同
    ...
    const get_results=()=> {
      ...
      if (state.is_first_load) {
        ...
        if (!lodash.isEmpty(homeName) && !lodash.isEmpty(awayName) && state.lang == lang) {
      ...
      // 判断是否是电竞
      if (utils.is_eports_csid(state.sport_id)) {
    ...
    const get_tr_detail=(data)=> {
      ...
      // 如果当前是电竞,就增加对应的请求参数
      if (utils.is_eports_csid(state.current_sport_id)) {
        params.isESport = "1";
      }
      // 清空上一次的请求结果
