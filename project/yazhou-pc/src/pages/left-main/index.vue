    ...
    >
      ...
      <template v-slot:header>
        ...
        <div class="menu-wrap scroll-fixed-bg relative-position bet_history">
          <!-- 投注记录 入口 -->
          <div
            v-show="MenuData.layout_left_show != 'bet_history'"
          ...
          <template v-if=" bet_count && !['bet_history'].includes(MenuData.layout_left_show) ">
            ...
            >
              ...
              <!-- <span class="text">
                  <template v-if="is_bet_single && ['today','play','early','hot_one','winner_top','hot'].includes(MenuData.cur_menu_type.type_name)">{{
                    i18n_t("bet.bet_one")
                  }}
            </template>

            <template v-else-if="MenuData.cur_menu_type.type_name=='bet'">
                {{ $t("bet.bet_n") }}
            </template>
            </span>
            <span class="bet-count">{{ bet_count }}</span> -->
