...
import { reactive, toRefs, onBeforeMount, watch,nextTick,getCurrentInstance } from "vue";
...
export const useTableData = ({ props,emit }) => {
 ...
 const item_class = (betResult)=> {
    if ([2,3,4,5,6,7,8,11,12,13,15,16,20].includes(parseInt(betResult))) {
 ...
 const matchType = (type, langCode = this.$i18n.locale)=> {
    ...
    if (type && langCode) {
      switch (parseInt(type)) {
        ...
        case 3:
          res = this.$root.$t(
            `common_lang.${langCode}.bet.champion_handicap`
          ); //"冠军盘赛事";
          break;
      }
    }
    ...
    if (lodash.get(state.money_obj, `${index}.money`)) {
      nextTick(() => {
        ...
        if (lodash.isArray(slider)) {
          ...
          state.early_settlement_data[index].computed_bet_amount = computed_bet_amount
 ...
 const change_slider = (val, index)=> {
    ...
    nextTick(() => {
      ...
      if (slider && slider[0] && lodash.isFunction(slider[0].getIndex)) {
        ...
        if (slider_index == -1) {
          ...
          if (obj_index[rat]) {
            slider_index = obj_index[rat];
            settlment_data['computed_bet_amount'] = (frontSettleAmount * rat) / 100
          } else {
            slider_index = 0;
            settlment_data['computed_bet_amount'] = frontSettleAmount * (min_money / max_money)
          }
          // 设置组件默认选中的进度
          slider[0].setIndex(slider_index);
        }
    ...
    if (
      ...
      computed_bet_amount = frontSettleAmount * (min_money / max_money);
        ...
        state.money_obj[ `${index}.money`] =rat === 100 ? max_money : Math.round(val)
 ...
 const show_bet_slide = (index)=> {
    ...
    state.early_settlement_data[index]['pre_settle_bet_amount_2'] = mathjs.subtract(preSettleBetAmount, money || 0)
  }
  /**
   * 部分结算后显示提前结算详情
   */
...
const  get_max_cashout = ()=> {
    ...
    lodash.forEach(list, (item, index) => {
      // 提前结算实时查询 是否包含该条数据
      let num = lodash.findIndex(props.orderNo_data_obj, { orderNo: item.orderNo });
      let bet_status = lodash.get(state.early_settlement_data[index], "bet_status");
      if (bet_status != "end_bet_pre") {
        if (num > -1) {
          ...
          if (
            ...
            // 注单剩余本金
            state.early_settlement_data[index].preSettleBetAmount = mathjs.subtract(
                orderAmountTotal,
              ...
              state.money_obj[`${index}.money`] =  lodash.get(state.early_settlement_data, `${index}.preSettleBetAmount`)
