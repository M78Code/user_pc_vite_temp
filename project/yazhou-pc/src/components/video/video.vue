...
-->
<template>
    <div class="video-wrap c-big-video v-scroll-area relative-position" :class="{'big-video-right':$route.params.video_size ==1}">
        ...
        <template v-if="show_video_replay || show_chatroom">
          <div class="video-bottom-panel" :class="{'iframe-video-bottom-panel': $utils.is_iframe}">
            <div class="panel-wrapper" :class="{'no-chatroom': !show_chatroom}">
              <div>
                <!-- 精彩回放 -->
                <video-history-line v-if="_.get(match_info,'cds')!='C01' && Number(match_info.csid) === 1" :match_info="match_info" :mid="mid" mode="Card" :mmp="+_.get(match_info,'mmp')" :matchTime="+_.get(match_info,'mst')" />
                <!-- 统计 -->
