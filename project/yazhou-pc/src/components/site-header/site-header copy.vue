        ...
        <div class="header-item item1 row items-stretch relative-position"
            ...
            <template>
                ...
                <template v-if="vx_get_left_menu_toggle">
                    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Necessitatibus, quod dolores? Facilis
                        excepturi ab vel dolore cum accusamus, tenetur sapiente exercitationem cupiditate aliquid, nemo
                        molestiae numquam obcaecati minima sequi magnam!</p>
...
function tab_click(obj) {
    ...
    if (menu.path.includes('/activity') && !get_global_switch.value.activity_switch) {
        // return this.$root.$emit(MITT_TYPES.EMIT_SHOW_TOAST_CMD, this.t("msg.msg_09"));
        return useMittEmit(MITT_TYPES.EMIT_SHOW_TOAST_CMD, t("msg.msg_09"))
    }
    // 电竞
    if (menu.id == 5) {
 ...
 */
function autoPlay() {
    currentSwipperIndex.value++
    if (currentSwipperIndex.value > currentSwipperArr.length - 1) {
        currentSwipperIndex.value = 0
    }
}
