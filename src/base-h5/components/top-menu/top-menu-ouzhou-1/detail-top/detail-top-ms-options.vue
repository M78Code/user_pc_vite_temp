...
<template>
  ...
  <nav class="option" @click="ChangeActive">
    <aside class="mhn team-image item">
      <!-- 左侧双打图标 type 0 表示主队,mhlu 主队的url -->
      <div class="figure">
        <teamImg :type="0" :url="detail_data.mhlu[0]" :fr="detail_data.frmhn[0]" :size="22" :csid="detail_data.csid"
                 style="margin-top: 0.11rem;"/>
        <teamImg v-if="detail_data.mhlu.length > 1" :type="0" :url="detail_data.mhlu[1]" :fr="detail_data.frmhn[1]"
                 :size="22" :csid="detail_data.csid" style="margin-top: 0.11rem; margin-left:-0.08rem;"/>
      </div>
      <p class="tips">{{ detail_data?.mhn }}</p>
    </aside>
    ...
    <aside class="man team-image item">
      <div class="figure">
        <!-- 右侧双打图标 type 1 表示客队,malu 客队的url  -->
        <team-img :type="1" :url="detail_data.malu[0]" :fr="detail_data.frman[0]" :size="22" :csid="detail_data.csid"
                  style="margin-top: 0.11rem;"/>
        <team-img v-if="detail_data.malu.length > 1" :type="1" :url="detail_data.malu[1]" :fr="detail_data.frman[1]"
                  :csid="detail_data.csid" style="margin-top: 0.11rem; margin-left:-0.08rem;"
                  :size="22"/>
      </div>
      <p class="tips">{{ detail_data?.man }}</p>
    </aside>
  </nav>
</template>
