 ...
 * @Description: 详情页橄榄球赛事阶段+赛事时间
-->
<template>
  <!-- 橄榄球 -->
  <span class='stage_child_14'>
    <span v-if="match_result_state">
      {{i18n_t('match_info.match_over')}}
    </span>
    <span v-else>
      {{i18n_t('mmp')[14][detail_data.mmp]}}
      <!-- 计时器 -->
  ...
  methods: {
    ...
    initRestTime(num){
      ...
      if(this.detail_data.mmp == '6' || this.detail_data.mmp == '7' || this.detail_data.mmp == '41' || this.detail_data.mmp == '42'){
        ...
        if(this.detail_data.mle == '0' || this.detail_data.mle == '2'){
          this.calculagraph_forward(num);
        }else{
          this.calculagraph_inversion(num);
        }
      }
    },
    /**
