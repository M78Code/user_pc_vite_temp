<template>
  <div class="virtual-sports">
    ...
    <div class="tab-wrapper">
      <div class="tab-item" v-for="(tab_item, i) of tab_items" :class="{ active: i == tab_item_i }" :key="i"
        @click="tab_item_click_handle(i, null, 'user_change')">
        <div>{{ tab_item.name }}</div>
      </div>
    </div>

    <div class="virtual-sports-container">
      <div class="tab-title">
        <div class="league-name right-border">{{ lengue_name }}</div>
        <div class="status">
          <!-- <span class="num">第10轮</span>
        <span class="state">比赛中</span> -->
          <icon-wapper class="icon" color="#333" name="icon-arrow" size="15px" />
        </div>
      </div>
      ...
      <template v-if="!no_virtual_match">
        ...
        <virtual-sports-stage :is_before_destroy="is_before_destroy" :key="current_match.mid"
          :m_status="current_match.match_status" :virtual_match_list="match_list_by_no" :current_match="current_match"
          source='list' :is_video_playing="is_video_playing" :v_match_router_ente="v_match_router_ente"
          :virtual_result_rank_data="virtual_result_rank_data" @time_ended="timer_ended_handle"
      ...
      ></match-tab> -->
        ...
        <div v-if="!ranking_list_change" class="v-sports-main-list"
          ...
          <div v-if="current_match.match_status == 0">
            <!-- 赛马切换玩法集tab组件 -->
            <virtual-sports-tab :batch="current_match_id" v-if="![1001, 1004].includes(current_sub_menu_id)">
            </virtual-sports-tab>
            <!-- 打印请勿删除 -->
            ...
            <div v-if="match_list_by_no && match_list_by_no.length && ![1001, 1004].includes(current_sub_menu_id)">
              <virtual-sports-category :top_menu_changed="top_menu_changed" :current_match="match_list_by_no[0]"
                source='sports' @top_menu_change="handle_top_menu_change" />
            </div>
          </div>
          ...
          <div class="v-sports-ranking" v-if="![1001, 1004].includes(current_sub_menu_id)">
            ...
            <result-page v-if="current_match.match_status == 2" :match_mid="current_match.mid"
              :current_match="current_match" @send_virtual_result_rank_data='send_virtual_result_rank_data' />
          </div>
          <!-- 注释勿删除 -->
        </div>
        <!-- 排行榜页面,小组赛淘汰赛页面  -->
        <div v-else class="list-wrapper">
          ...
          <div v-if="[1001, 1004].includes(current_sub_menu_id)">
            <!--  足球小组赛,淘汰赛页面  -->
            <group-knockout v-if="tab_items[tab_item_i] ? tab_items[tab_item_i].field3 != '' : false"
              :tid="VirtualData.menu_list[tab_item_i].field1" :current_match="current_match" />
            <!--  足球排行榜页面  -->
