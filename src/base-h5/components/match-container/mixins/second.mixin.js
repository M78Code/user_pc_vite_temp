...
export default defineComponent({
  ...
  methods:{
    ...
    async overtime_tab_handle(item, unfold, operate_type, sub_i){
      ...
      if(item.unfold == 1){
        ...
        if(operate_type == 'is-user' || operate_type == 'mounted'){
          await MatchMeta.get_match_base_hps_by_mids({ mids: this.match.mid, source: 'attached', other: {
            pids:item.pids,
          ...
          this.compute_secondary_play(item)
        }
      }
      if(item.id==17){
    ...
    compute_secondary_play () {
      ...
      if([18].includes(id)){
        ...
        //次要玩法展开或者关闭通知列表页重新计算dom高度
        this.save_second_play_mid_map_unfold_status(this.current_tab_item, this.bold_all_list, this.five_minutes_all_list);
    ...
    apply_15min_title(){
      ...
      if(this.current_tab_item.id==17){
        ...
        const hps_data = lodash.cloneDeep(hps15Minutes).sort((a, b) => +a.hSpecial - +b.hSpecial)
        ...
        if (length > 0) {
          ...
          hSpecial=lodash.get(hps_data,'[0].hSpecial',1) - 1;
        } else {
          const time = Math.floor(Number(this.match.mst) / 60)
          hSpecial = Math.floor(time / 15)
        }
        if(standard_odd_status==1 && length == 6){ // 翻转后取第二个值
          hSpecial=lodash.get(hps_data,'[3].hSpecial', standard_odd_status + 1)-1;
        }
        if(hSpecial>4){
  ...
  computed:{
    ...
    show_tab_by_data(){
      ...
      return lodash.get(this.match, 'is_show_secondary_play', false)
