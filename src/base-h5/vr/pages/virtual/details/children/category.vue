...
export default {
  ...
  computed:{
    ...mapGetters([
      ...
      'get_details_tabs_list',
      'get_chpid_obj',
      'get_filter_electronic_show'
    ]),
    show_recommend(){
      let flag = false;
      if(!this.is_loading && this.is_no_data){
        if(this.$route.name != 'match_result'){
          if(this.get_details_item){ //当前玩法下无数据就显示
            flag = true;
          }
        }
      }
    ...
    // 赛事id
    match_id() {
      return  this.get_goto_detail_matchid || this.get_detail_data.mid || this.$route.params.mid
    },
  },
  ...
  watch:{
    '$route' (to, from) {
      ...
      if (
          this.get_menu_type !== 28 && !to.query.search_term && to.params.mid === from.params.mid
          || to.query.search_term && this.match_play_item_changed
      ) {
        this.initEvent();
      }
      // 当切换玩法集的时候变为: true
      this.first_load = true;
      this.match_play_item_changed = true
    },
    // 监听赛事id的变化 如果赛事id变化 及时更新调用玩法集合的接口
    get_goto_detail_matchid(){
      if(this.matchInfoCtr){
        this.matchInfoCtr.destroy()
      }
      this.matchInfoCtr = new MatchInfoCtr(this);
    },
    // 监听get_fewer的值
    get_fewer(n){
      if(n != 3){
  ...
  created(){
    ...
    this.set_fewer(1);
    // 只有赛果详情才调用相应接口
    const paramIndex = this.$route.params.index
    if (+paramIndex === 0 || (this.get_menu_type === 28 && this.$route.name === 'match_result')) {
      this.initEvent().then(() => {
        // 获取赛果数据后，滑动到顶部
    ...
    if(!['result_details', 'match_result'].includes(this.$route.name)){
      ...
      let params = {
        ...
        // userId或者uuid
        cuid: this.get_uid,
        round: this.get_menu_type == 3000 ? (this.get_details_tabs_list && this.get_details_tabs_list[this.get_subscript_game_index] && this.get_details_tabs_list[this.get_subscript_game_index].round) : null
      }
  ...
  methods: {
    ...
    change_minheight() {
      if (this.$refs.category) {
        // 0.44 + 0.4 + 0.4 = 1.24
        let val = [1, 2].includes(+this.get_detail_data.csid) ? 1.24 : 0.84

        // 横屏 或 赛事盘口全部关闭 且无推荐赛事 时 不设置最小高度
        if (
            this.get_is_hengping ||
            this.show_recommend && !this.$refs.detail_match_list
        ) {
          this.$refs.category.style.minHeight = 'unset'
        } else {
          this.$refs.category.style.minHeight = window.innerHeight - this.rem(val) + 'px'
        }
      }
    },
    // 点击玩法名称,隐藏或是显示玩法赔率
    change_show(){
      ...
      if(Array.isArray(this.matchInfoCtr.list) && this.matchInfoCtr.list.length){
        ...
        let obj_ = {}
        this.matchInfoCtr.list.forEach( (v_item, v_index)=> {
          obj_.topKey = v_item.topKey
          obj_.hshow = v_item.hshow
          this.arr_hshow.push(obj_)
          obj_ = {}
        })
        if(flag1){
          this.set_fewer(1)
    ...
    get_details_data_cache_fillter(all_data){
      const mcid =  this.get_details_item || (this.$route.params.csid?'':this.$route.params.mcid)
      const findItme = this.get_details_tabs_list.find( item=>item.id == mcid )
      // console.log(findItme,"findItme");
      const { plays= [],round='' } = findItme

      // console.log(this.get_chpid_obj,"set_chpid_objset_chpid_obj");
      const res = all_data.filter( item=>{
        // 电竞需要判断第一局和第二局的原因，需要加上chpid判断
        if( this.get_menu_type == 3000 || this.get_filter_electronic_show){
          if( round ){
            // console.log( this.get_chpid_obj[`${item.hpid}-${round}`] ,round, item.chpid , item.hpid,"dsjfkldskjfs");
            // console.log(this.get_chpid_obj[`${item.hpid}-${round}`] , item.chpid,"this.get_chpid_obj[`${item.hpid}-${round}`] == `${item.hpid}-${round}`");
            return plays.includes( +item.hpid ) && ( this.get_chpid_obj[`${item.hpid}-${round}`] == item.chpid || item.chpid === item.hpid)
          }else{
            // 所有投注
      ...
      // console.log(all_data,"this.all_dataall_dataall_data.all_dataall_data");
      return res
    },
     ...
     */
    async initEvent(to_refresh, init_req){
      if(to_refresh) {
      ...
      let params = {
        ...
        // mcid: ['result_details', 'match_result'].includes(this.$route.name) ? 0 : this.get_details_item || (this.$route.params.csid?'':this.$route.params.mcid), // 玩法集id
        // 2023/3/4 普通赛事,电竞详情拉取所有玩法集数据
        mcid:0,
        mid: this.match_id, // 赛事id
        cuid: this.get_uid, // userId或者uuid
        // round: this.get_menu_type == 3000 ? (this.get_details_tabs_list && this.get_details_tabs_list[this.get_subscript_game_index] && this.get_details_tabs_list[this.get_subscript_game_index].round) : null
        round:null
      }
      // 如果是 赛果下边的 电竞，则加 isESport 参数
      if(this.get_menu_type == 28 && [3001,3002,3003,3004].includes(+this.get_curr_sub_menu_type)){
        params.isESport = 1
      }else{
        params.isESport = null
      }

      this.is_loading = to_refresh !== 'hide_loading'

      const tabs_active_data_cache = this.get_details_data_cache[`${this.match_id}-${this.get_details_item}`]
      if(tabs_active_data_cache) {
        this.is_no_data = false;
      }
      // 调用: /v1/m/matchDetail/getMatchOddsInfoPB接口
      //赛果页面调用赛果玩法详情接口
      let http = ['result_details', 'match_result'].includes(this.$route.name) ? api_result.get_match_result :
          (this.get_menu_type == 3000 || this.get_filter_electronic_show ? api_common.get_DJ_matchDetail_getMatchOddsInfo : api_common.get_matchDetail_getMatchOddsInfo)
      ...
      if( !to_refresh &&  !['result_details', 'match_result'].includes(this.$route.name) && Object.keys(this.get_details_data_cache).length && this.get_details_data_cache[`${this.match_id}-0`] && this.get_details_data_cache[`${this.match_id}-0`].length ){
        // 将 get_details_data_cache 中所有投注得信息的数据给details_data_cache
        details_data_cache[`${this.match_id}-0`] = JSON.parse(JSON.stringify(this.get_details_data_cache[`${this.match_id}-0`]))
        temp = this.get_details_data_cache_fillter(details_data_cache[`${this.match_id}-0`])
      ...
      if(temp.length === 0){
        try {
          ...
          const res = await this.axios_api_loop(_obj)

          if(this.send_gcuuid != res.gcuuid) {
            return;
          }
          ...
          details_data_cache[`${this.match_id}-0`] = data
          // chipid进行处理
          const chpid_obj = {}
          data.forEach(item=>{
            if( item.chpid ){
          ...
          this.set_chpid_obj(chpid_obj)
          
          if (['result_details', 'match_result'].includes(this.$route.name)) {
            temp = details_data_cache[`${this.match_id}-0`]
          } else {
            temp = this.get_details_data_cache_fillter(details_data_cache[`${this.match_id}-0`])
          }
        } catch (error) {
      ...
      try {
        ...
        if(temp&&temp.length){
          if(this.to_refresh == 'details_refresh' && this.arr_hshow.length > 0){
            this.save_expanded_state(temp)
          }
              ...
              item.hl.forEach(hls_array => {
                if(_.isArray(hls_array.ol)){
                  hls_array.ol.forEach(ol_item => {
                    ol_item.csid = this.get_detail_data.csid;
                  });
                }
              });
            }
        ...
        // 当前玩法集下数据缓存和所有的投注项
        details_data_cache[`${this.match_id}-${this.get_details_item}`] = temp
        this.set_details_data_cache(details_data_cache)
      }catch(err){
        console.error(err);
      }finally{
        if(is_cache){
        ...
        if(!['result_details', 'match_result'].includes(this.$route.name)){
          this.$root.$emit(this.emit_cmd.EMIT_MATCHINFO_LOADING, true)
        }
        const tabs_active_data_cache = this.get_details_data_cache[`${this.match_id}-${this.get_details_item}`]

        // 当前赛事对应玩法集存在缓存数据
        if (tabs_active_data_cache) {
          this.matchInfoCtr.setList(_.cloneDeep(tabs_active_data_cache))
        } else {
          // 无数据
          this.is_no_data = true;
      ...
      //   // 当前玩法集下数据缓存
      //   const details_data_cache = {
      //     [`${this.match_id}-${this.get_details_item}`]: temp
      //   }
      //   this.set_details_data_cache(details_data_cache)
      // })
      //   .catch(err=>{
     ...
     * @return {*}
     */
    async axios_api_loop({axios_api,params,fun_then=null,fun_catch=null,max_loop=3,timers=1000, loop_count=0,timer=0,new_params}){
      ...
      return new Promise((resolve,reject) => {
        ...
        axios_api(params).then(res => {
          ...
          if(loop_count++>=(max_loop-1)){
            ...
            timer=setTimeout(() => {
              this.axios_api_loop({axios_api,params,fun_then,fun_catch,max_loop,timers,loop_count,timer});
            }, timers);
          }
        });
      })
    ...
    // 是否隐藏详情热门推荐
    hide_detail_match_list(flag) {
      if (flag) {
        this.no_recommend_match_list = true
      }
    },
    // 调用 玩法集列表接口
    triggle_tabs_update(){
    ...
    // 保存当前展开状态
    save_expanded_state(arr_list) {
      arr_list.forEach(v_item => {
        this.arr_hshow.forEach(v_m => {
    ...
    socket_upd_list(skt_data,callback){
      ...
      let params = {
        ...
        // userId或者uuid
        cuid: this.get_uid,
        round: this.get_menu_type == 3000 || this.get_filter_electronic_show  ? (this.get_details_tabs_list && this.get_details_tabs_list[this.get_subscript_game_index] && this.get_details_tabs_list[this.get_subscript_game_index].round) : null
      }
      // 如果是 赛果下边的 电竞，则加 isESport 参数
      if(this.get_menu_type == 28 && [100,101,102,103].includes(+this.get_detail_data.csid)){
        params.isESport = 1
      }else{
        params.isESport = null
      }
      ...
      //赛果页面调用赛果玩法详情接口
      let http = ['++', 'match_result'].includes(this.$route.name) ? api_result.get_match_result :
          (this.get_menu_type == 3000 || this.get_filter_electronic_show ? api_common.get_DJ_matchDetail_getMatchOddsInfo : api_common.get_matchDetail_getMatchOddsInfo)
      this.send_gcuuid = uid();
      params.gcuuid = this.send_gcuuid;
      http(params).then(res=>{
        ...
        const tabs_active_data_cache = this.get_details_data_cache[`${this.match_id}-${this.get_details_item}`]
          console.log('tabs_active_data_cache',res);
          // 当前赛事对应玩法集存在缓存数据
          if (tabs_active_data_cache) {
            this.matchInfoCtr.setList(_.cloneDeep(tabs_active_data_cache))
          } else {
            // 无数据 如果走ws协议 盘口数据接口报错 不需要处理 取上一次成功获取的盘口数据 处理正常返回盘口的数据为空的时候
            if(res.code == 200){
              this.is_no_data = true;
              this.matchInfoCtr.setList([])
            }
        ...
        // .finally(() => {
        //   const tabs_active_data_cache = this.get_details_data_cache[`${this.match_id}-${this.get_details_item}`]
        //   console.log(tabs_active_data_cache,'tabs_active_data_cache');
        //   // 当前赛事对应玩法集存在缓存数据
        //   if (tabs_active_data_cache) {
        //     this.matchInfoCtr.setList(_.cloneDeep(tabs_active_data_cache))
        //   } else {
        //     // 无数据 如果走ws协议 盘口数据接口报错 不需要处理 取上一次成功获取的盘口数据
        //     this.is_no_data = true;
        //     this.matchInfoCtr.setList([])
        //   }
        // })
    ...
    save_hshow(temp,list_old){
      ...
      if(list_old)
      {
        ...
        middle_data = _.cloneDeep(this.matchInfoCtr.list);
      }
      let middle_obj = {}
    ...
    getdetail_cache_session(params){
      ...
      if(cach_string){
        ...
        this.matchInfoCtr.setList(_.cloneDeep(data));
      }
      return cach_string;
    },
    ...
    on_listeners() {
      ...
      this.$root.$on(this.emit_cmd.EMIT_HIDE_DETAIL_MATCH_LIST, this.hide_detail_match_list)
    },
    // 移除相应监听事件
    off_listeners() {
      ...
      this.$root.$off(this.emit_cmd.EMIT_HIDE_DETAIL_MATCH_LIST, this.hide_detail_match_list)
    }
  },
  /**
  ...
  destroyed(){
    this.off_listeners();
    this.debounce_throttle_cancel(this.socket_upd_list);

    // 清除数据避免下次进来产生干扰
    this.set_details_data_cache({})

    if(this.matchInfoCtr) {
      this.arr_hshow = []
      this.matchInfoCtr.clearData()
      this.matchInfoCtr.destroy()
    }
  }
}
