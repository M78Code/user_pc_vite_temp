    ...
    <div class="bet-success-bar yb_px14 yb_py14">
      ...
      <div class="row justify-between items-center">
        ...
        <p class="yb_fontsize14 text-right status-mark" :style="calc_color">
          <!-- 投注成功 -->
          <span v-if="order_tatus == 1" class="color1"><img  src="image/wwwassets/bw3/svg/bet_chengg.svg">{{i18n.t('bet.bet_suc')}}</span>
          <!-- 投注失败 -->
      ...
      <div class="row justify-between items-center yb_fontsize14 money-d">
        <!-- 最高可赢 -->
        <p>{{i18n.t('bet.total_win2')}}：<span :class="len == 1?'color4':'color5'"><template v-if="len == 1"></template>{{max_win_money}}</span></p>
        <!-- 投注金额 -->
        <p class="text-right">{{i18n.t('bet.bet_val')}}：<span class="color6"><template v-if="len == 1"></template>{{(item_.seriesBetAmount / 100).toFixed(2)}}</span> </p>
    </div>
  </div>
</template>
