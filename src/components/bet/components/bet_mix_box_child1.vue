        ...
        <template v-if="get_bet_list.length == 1">
          <template v-if="BetData.is_bet_success_status">
            ...
            <div class="row justify-between yb_px14 yb_fontsize14 yb_mb8 bottom-bar">
              <p><span>{{ i18n.t('bet_record.bet_max_win') }}</span><span class="yb_fontsize14 yb_ml8 bottom-bar-sp">{{
                (max_winmoney / 100).toFixed(2) }}</span></p>
              <p><span>{{ i18n.t('bet.bet_val') }}</span><span class="yb_fontsize14 yb_ml8 bottom-bar-sp2">{{ (bet_money
...
watch(() => get_order_ing.value.length, (new_, old_) => {
  if (new_ == 0 && old_) {
    if (get_order_los.value.length) {  //所有订单确认时，如果新流程有失败的订单回到状态1
      ...
      if (is_new_bet.value) {
        if (!tips_msg) {
          tips_msg = i18n.t('bet.err_msg03')  //新流程queryOrderStatus 接口返回的投注失败提示语先写死
        }
