          ...
          <template v-if="bet_this.bet_flag">
            ...
            <div class="full-width bet-foot-menu">
              <div class="row check-bet-prefer cursor-pointer" :class="{ 'checked': bet_this.user_bet_prefer }"
                ...
                <!--底部菜单部分-->
                <check-box :checked="bet_this.user_bet_prefer" /> {{ $t('bet.bet_auto_msg_1') }}
              </div>
              <div class="row check-box cursor-pointer" :class="{ 'checked': bet_this.is_common_amount }"
                ...
                <check-box :checked="bet_this.is_common_amount" /> {{ $t('bet.common_amount') }}
              </div>
            </div>
          </template>
          <template v-else>
            <div class="full-width bet-foot-complete"
              ...
              <div class="full-width cursor-pointer"
                ...
                <template v-if="bet_this.bet_loadding">
                  <span class='bet-confirming'>{{ $t('bet.bet_loading') }}</span><q-spinner-dots size="16"
            ...
            <div class="full-width pt15">
              <div class="row bet-total">
                ...
                <div class="col-auto bet-total-right">
                  {{ format_currency(bet_this.bet_total_money) }}
                </div>
              </div>
              <div class="row bet-total">
                <!-- 预计总收益 -->
                ...
                <div class="col-auto bet-total-right bet-gold-text">
                  {{ format_currency(bet_this.bet_total_win_money) }}
                </div>
              </div>
              <q-separator class="bet-mix-separator"></q-separator>
              ...
              <div class="bet-message text-center">
                <template v-if="bet_this.BetData.bet_list.length < bet_this.BetData.mix_min_count">
                  <!--至少选择2场比赛-->
                  {{ $t("bet.bet_min_item").replace('%s', bet_this.BetData.mix_min_count) }}
                </template>
                <template v-else-if="bet_this.BetData.bet_list.length > bet_this.BetData.mix_max_count">
                  {{ $t("bet.bet_max_item").replace('%s', bet_this.BetData.mix_max_count) }}
                </template>
                <template v-else>
                  <!--错误提示-->
                  {{ bet_this.view_ctr_obj.error_message }}
