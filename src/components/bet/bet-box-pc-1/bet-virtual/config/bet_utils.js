 ...
 * @param: csids             需要显示3位小数的赛种集合

 * @return:可能组合的对象
 * @Date 2019/12/21 15:15:50
 */
import BetCountJointNumber from "src/public/utils/bet/betCountJointNumber.js"
export function get_max_win_money(bet_list, bet_obj, bet_s_list, series_values, bet_amount, csids) {
  ...
  if (bet_s_list && bet_s_list.length && series_values && bet_s_list && bet_s_list.length) {
    ...
    if (p0 < p1) {
      ...
      let sa_len = series_assemble.length;
      for (let i = 0; i < sa_len; i++) {
    ...
    for (let i = 0; i < series_list.length; i++) {
      ...
      for (let j = 0; j < keys.length; j++) {
        ...
        if (cs) {
          ...
          if (odds_value_ && odds_value_ != '') {
            if(csids && csids.includes(1*cs.sport_id))
            {
              float3 = true;
              odds_value = ((Math.floor(odds_value_ / 100)) / 1000) * odds_value;
            } else {
              odds_value *= odds_float_format2(parseFloat(odds_value_) / 100000);
            }
          }
        }
      ...
      if(float3){
        odds_value = ((Math.floor(odds_value * 1000)) / 1000).toFixed(3);
      }
      //串关所有赔率相乘，直接保留两位小数
