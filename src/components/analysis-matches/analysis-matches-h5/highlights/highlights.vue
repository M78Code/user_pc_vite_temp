        ...
        <template v-if="get_menu_type === 28">
          <div class="row game-over">
            ...
            <div class="item-content real-time-contv-ifent hairline-border">
              ...
              <span class="time" v-else>{{  format_mgt_time(lodash.get(get_detail_data, 'mststr', 0))     }}</span>
          ...
          <div class="item-content hairline-border"  @click="handle_click_event(i, event)">
            ...
            <div class="text-wrapper">
              ...
              <div class="text-scroller" v-else>
                ...
                <!--<span class="text">{{event.homeAway}} {{ $t(`highlights.event_type.${event.eventCode}`, {X: trans_num(event.firstNum)})}}</span>-->
              </div>
            </div>
...
import { format_total_score ,format_mgt_time  } from "src/core/index.js";
// components: {
  // 队标视图
  ...
  const replay_video_src = computed(() => {
      ...
      const host_url = window.BUILDIN_CONFIG.live_domains[0] || loadsh.get(get_user,'oss.live_h5')
