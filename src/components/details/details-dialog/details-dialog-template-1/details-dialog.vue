...
export default {
  ...
  methods: {
    ...
    change_active(item) {
      ...
      this.$root.$emit(this.emit_cmd.EMIT_REFRESH_DETAILS); // 刷新详情页头部信息;
      this.$root.$emit(this.emit_cmd.EMIT_REFRESH_DETAILS_TAB); // 将tab的滚动距离回复到初始点;
      this.$root.$emit(this.emit_cmd.EMIT_CATEGORY_SKT); // 底部信息skt连接
      this.$root.$emit(this.emit_cmd.EMIT_DETAILS_SKT); // 头部信息skt连接
      this.timer1_ = setInterval(()=>{
        this.$root.$emit(this.emit_cmd.EMIT_MATCH_TIME_SHOW_INIT);
        this.$root.$emit(this.emit_cmd.EMIT_UPDATE_GAME_TIME)
