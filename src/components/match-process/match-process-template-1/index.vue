<template>
  <div class="c-match-process text-center" :class="rows == 1 ? 'row a-row' : 'column'">
    <div v-show="match_props.match.mcid && show_page == 'match-list'" class="jingcai">
      {{ match_props.match.mcid }}
    </div>
    <!-- ms值3-比赛结束 4-比赛关闭 -->
    <div v-show="$utils.get_match_status(match_props.match.ms) || [3, 4].includes(1 * match_props.match.ms) ||
      (match_props.match.mcid && match_props.match.mmp != 0)
      " class="process-name" v-html="computed_process_name">
    </div>
    <!--补充时间-->
    ...
    <template v-else> -->
    <match-date :rows="date_rows" v-if="computed_show_date" :match_props="match_props" :match_list_data="match_list_data"
      class="date-wrap" />
...
watch(computed_process_name, () => {
  score_switch_handle(this.match_props.match)
}, { immediate: true })

watch(show_fill_time, () => {
  init_fill_time()
})

watch(match_props.match.mstst, () => {
  init_fill_time();
})

watch(match_props.match.mststi, () => {
  init_fill_time();
})
...
const init_fill_time = (skt_mid) => {
  ...
  // 补充时间(倒计时部分)
  cur_fill_time.value = _.get(this.match_props, 'match.mstst') || 0;
  // 当前阶段对应的正常时间(在第多少分钟时结束)
  cur_mmp_time.value = this.mmp_time_obj[csid][mmp];
  // 补充时间(+分钟部分) (补时多少分钟)
  cur_fill_second.value = _.get(this.match_props, 'match.mststi') || 0;
}
