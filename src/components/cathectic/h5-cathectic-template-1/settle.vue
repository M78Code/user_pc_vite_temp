      ...
      <scroll ref="myScroll" :on-pull="onPull" v-else>
        <div class="edit row items-center yb_fontsize12">
            <div class="time yb_mr6 relative-position" @click="change_date">
              <i class="calendar"></i><span>{{date_limit == 7 ? i18n_t('bet_record.7day') : i18n_t('bet_record.30day')}}</span>
              </div>
            <div class="sort relative-position" @click.stop="change_sort($event)">
              ...
              <!-- <p v-if="is_sort_show && 0" class="absolute">
                <span class="sort-text" :class="{'select': sort_active == 2}" data-num='2'><i class="sort0" :class="{'sort-2':sort_active == 2}"></i>{{ $t('bet_record.sort3') }}</span>
                <span class="sort-text" :class="{'select': sort_active == 1}" data-num='1'><i class="sort1" :class="{'sort-1':sort_active == 1}"></i>{{ $t('bet_record.sort4') }}</span>
                <span class="sort-text" :class="{'select': sort_active == 3}" data-num='3'><i class="sort2" :class="{'sort-3':sort_active == 3}"></i>{{ $t('bet_record.sort5') }}</span>
              </p> -->
            </div>
            <div>
            ...
            <div v-for="(value,name,index) in list_data" :key="index">
              <template v-if="!is_early|| (is_early && clac_is_early(value.data))">
                ...
                <p class="tittle-p row justify-between yb_px4" :class="{'tittle-p2':index == 0}" @click="toggle_show(value)">
                  <span>{{(new Date(name)).Format(i18n_t('time2'))}}</span>
                  <span class="betamount" v-show="get_main_item == 1 && value.open">{{ $t('bet.number_transactions') }}<span
                      class="color-1 yb_m">{{value.totalOrders}}</span>&emsp;{{ $t('bet.betting') }}<span
                      class="color-1">{{value.betAmount}}</span>&emsp;{{ $t('bet_record.bet_no_status03') }}/{{ $t('bet_record.bet_no_status04') }}<span class="color-1"
                ...
                <q-slide-transition>
                  <div v-show="value.open">
                    <!-- 投注记录的每一个卡片 -->
                    <common-cathectic-item :item_data="item2" v-for="(item2,key) in value.data" :key="key" class="yb_my12" :key2="key" :len="value.data.length"  :is_early="is_early"></common-cathectic-item>
                  </div>
                </q-slide-transition>
              </template>
            </div>
