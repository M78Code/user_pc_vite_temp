...
<template>
    <div class="mx-10 pre_record" ref="pre_record">
        ...
        <div class="filter-button" v-if="get_user.settleSwitch == 1">
            ...
            {{ $t('pre_record.expired')}}<i class="early yb_ml4" :class="{'early2': selected_expired}"></i>
            </i>
        </div>
...
const cancle_pre_order = () => {
    ...
    if(res.code == 200){
        // TODO: 从vuex取待改造
        // set_toast({ 'txt': i18n_t('pre_record.canceled'), hide_time: 3000 });
        ...
        cancle_confirm_pop_visible.value = false
        // TODO: 从vuex取待改造
        // set_toast({ 'txt':res.code == '0400546'? i18n_t('pre_record.cancle_fail_tips'):i18n_t('pre_record.cancle_fail_tips2'), hide_time: 3000 });
    }
    }).catch(()=>{
    cancle_confirm_pop_visible.value = false
    })
