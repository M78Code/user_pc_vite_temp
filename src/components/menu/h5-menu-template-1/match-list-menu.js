...
export default {
  methods: {
    ...
    on_listeners() {
      useMittOn(
        MITT_TYPES.EMIT_MENU_MATCH_COUNT_CHANGE,
        this.menu_match_count_change_on
      ).on;
      useMittOn(
        MITT_TYPES.EMIT_RE_STATISTICS_MATCH_COUNT,
        this.re_statistics_match_count_on
      ).on;
    ...
    record_main_index(main_index) {
      ...
      if (lodash.get(this.get_current_first_menu, "menuType") == 1) {
        main_menu_first_i = 0;
      } else if (lodash.get(this.get_current_first_menu, "menuType") == 3000) {
        main_menu_first_i = 2;
      } else if (lodash.get(this.get_current_first_menu, "menuType") == 900) {
        main_menu_first_i = 3;
      }
    ...
    initial_loading_of_the_first_level_menu() {
      ...
      if (this.get_sport_all_selected) {
        ...
        this.sub_menu_i = lodash.findIndex(this.sub_menu_list, {
          menuId: this.get_current_sub_menuid,
        });
    ...
    get_main_menu_init_data() {
      ...
      m_list.forEach((m_m) => {
        ...
        else if (m_m.menuType == 900) {
          virtual = m_m;
        } // 虚拟体育
        else if (m_m.menuType == 3000) {
          esport = m_m;
        } // 電競
        else {
          this.main_select_items.push(m_m);
        } // 中间的 一级菜单
      });
      let middle =
        this.main_select_items[this.selector_w_m_i ? this.selector_w_m_i : 0]; // 当前选中的 中间一级菜单
    ...
    find_second_menu_have_first_match() {
      ...
      for (let i = 0, len = new_main_menu_list_items.length; i < len; i++) {
        if (new_main_menu_list_items[i].ct > 0) {
          // 二级菜单其中一个有数量，赋值下标
    ...
    check_selected_is_nomatch() {
      ...
      // 查找当前 二级菜单
      let found_sub = this.sub_menu_list.filter(
        (sub_menu) => sub_menu.menuId == cu_sub_menuid
      )[0];
      if (found_sub) {
    ...
    re_statistics_match_count_on() {
      if (["category", "details", "virtual_sports"].includes(useRoute().name)) {
        return;
      }
    ...
    menu_match_count_change_on(skt_data) {
      ...
      main_menu_array.forEach((main, m_i) => {
        ...
        main.subList.forEach((sub) => {
          let found_sub = skt_data.filter(
            (skt_sub) => skt_sub.menuId == sub.menuId
          )[0];
          if (found_sub) {
    ...
    get_the_first_and_second_level_menu_id() {
      let first_menu_i = -1,
        main_data_i = 0;
      // mt1,mt2 转换为m,s,t参数
      if (Object.keys(this.get_global_route_menu_param).length) {
        //首页跳转菜单参数
        this.set_global_route_menu_param(
          this.menu_type2menu_id(
            this.get_global_route_menu_param,
            this.main_menu_list
          )
        );
      }
      ...
      if (main_id) {
        main_data_i = lodash.findIndex(this.main_menu_list, {
          menuId: main_id,
        });
        // 如果没有找到，默认 0
        if (main_data_i < 0) {
      ...
      if (menu_data && menu_data.length) {
        ...
        if (route_menu_param.mt1 && route_menu_param.mt2) {
          ...
          for (let i = 0; i < menu_data.length; i++) {
            if (
              route_menu_param.mt1 == lodash.get(menu_data, i + ".menuType")
            ) {
              ...
              if (menu_subList) {
                for (let j = 0; j < menu_subList.length; j++) {
                  if (
                    route_menu_param.mt2 ==
                    lodash.get(menu_subList, j + ".menuType")
                  ) {
    ...
    async get_date_menu_api_when_subchange(item) {
      // 如果是早盘，串关，电竞的话
      if (
        [4, 11, 3000].includes(+this.menu_type) &&
      ...
      } else if ([28].includes(+this.menu_type)) {
        // 如果是赛果
        this.date_menu_list = this.sub_menu_list[this.sub_menu_i].subList;
        // 设置日期选中项 调用三级菜单点击事件，默认第一个
        this.select_result_date_menu();
    ...
    set_one_two_three_level_menu_data() {
      ...
      // 设置2级菜单类型
      if (this.get_current_second_menu) {
        this.set_curr_sub_menu_type(
          lodash.get(this.get_current_second_menu, "menuType")
        );
      }
      if (this.get_current_three_menu) {
        this.set_md(this.get_current_three_menu.field1);
        // 设置3级菜单id
        this.set_curr_third_menu_id(
          lodash.get(this.get_current_three_menu, "menuId")
        );
      }
      this.set_current_menu(menu_record);
    },
    ...
    get_main_dom_i_handle(menu_id) {
      let main_dom_i = 0;
      if (menu_id) {
        main_dom_i = lodash.findIndex(this.main_menu_list_items, {
          menuId: menu_id,
        });
        if (main_dom_i == -1) {
          main_dom_i = 1;
  ...
  computed: {
    ...
    first_level_menu_subscript_css() {
      return {
        yb_fontsize12:
          ["th", "ms", "vi", "ad"].includes(this.get_lang) ||
          (["en"].includes(this.get_lang) &&
            this.get_newer_standard_edition == 1),
      };
    },
    // 赛果下边的 虚拟体育 的四级菜单 数据
