<template>
  <div class="basic-wrap relative-position" @click.stop="on_go_detail" >
    ...
    <div class="row-item team-item">
      <div class="team-logo" :class="_.get(match,'match_logo.is_double',false) && 'double-logo'">
        ...
        <!-- 双打局，就会有两个头像 -->
        <img style="width: 22px; max-height: 24px;" class="logo2" v-if="_.get(match,'match_logo.is_double',false)" v-img="[(match.match_logo || {}).home_2_logo,(match.match_logo || {}).home_2_letter,match.csid]" />
      </div>
      <div class="ellipsis-wrap">
        <div class="row no-wrap absolute-full">
          <div class="team-name home ellipsis allow-user-select" :class="{'bold':match.team_let_ball=='T1'}" v-tooltip="{content:_.get(match,'mhn',''),overflow:1}">{{match.mhn}}</div>
        </div>
      </div>
      <!-- 主比分 -->
      <div class="score">
        <span v-show="!scoring">{{match.home_score}}</span>
        <span v-show="scoring" class="scoring">{{ $root.$t('mmp.100.scoring') }}</span>
      </div>
    </div>
    <!-- 客队信息 -->
    <div class="row-item team-item">
      <div class="team-logo" :class="_.get(match,'match_logo.is_double',false) && 'double-logo'">
        ...
        <img style="width: 22px; max-height: 24px;" class="logo2" v-if="_.get(match,'match_logo.is_double',false)" v-img="[(match.match_logo || {}).away_2_logo,(match.match_logo || {}).away_2_letter,match.csid]" />
      </div>
      <div class="ellipsis-wrap">
        <div class="row no-wrap absolute-full">
          <div class="team-name away ellipsis allow-user-select" :class="{'bold':match.team_let_ball=='T2'}"  v-tooltip="{content:_.get(match,'man'),overflow:1}">{{match.man}}</div>
        </div>
      </div>
      <!-- 主比分 -->
      <div class="score">
