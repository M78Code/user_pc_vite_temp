<!--
 * @Author: Cable
 * @Date: 2021-08-06 21:27:29
 * @Description: 赛事玩法模板0
-->
<template>
  <div
    class="c-match-item  match-tpl0-bg"
    :class="{'more-handicap':match.has_add1 || match.has_add2}"
  >
    <!-- 比赛进程 -->
    <div class="process-col yb-flex-center">
      ...
      <img class="match-hot" src="~public/image/common/svg/hot.svg" v-if="match.is_hot"/>
       <!-- 比赛进程 -->
        <match-process v-if="is_mounted && match.api_update_time !=0" :match_props="{match, source: 'match_list'}" show_page="match-list" :rows="2" :match_list_data="match_list_data" />
    </div>
    ...
    <div class="match-handicap-item-wrap">
      ...
      <div class="other-play-tab" v-if="match.has_other_play">
        ...
        <div class="play-title col" @click="fold_tab_play" :style="`width:${match_list_tpl_size.team_width+match_list_tpl_size.bet_width *( match.tpl_id ==13 ? 13   : 6  )}px !important;flex:none`">
          ...
          <tab
            :list="play_name_list"
            :padding="10"
            :currentIndex="match.play_current_index"
            tab_name_key="play_name"
      ...
      <!-- 次要玩法标题 -->
      <div :class="['fifteen-box',{'double-title':['en','ad','ms'].includes(get_lang)}]" v-if="match.has_other_play && !match_style_obj.is_fold_tab_play">
          ...
          <div class="row">
            <div class="handicap-col fifteen-item bet-item-wrap fifteen_tab_txt"
                 ...
                 v-for="(item,key) in bet_col">
                <div class="double-row" v-if="item.includes('%n')">
                <div v-for="(text,i) in item.split('%n')" :class="[{'highlight-t': i === 1 && [3,4,5].includes(key)}]" :key="i">{{text}}</div>
              </div>
              <template v-else>
                {{item}}
              </template>
      ...
      <div class="match-handicap-item other-handicap-item" v-if="match.has_other_play && !match_style_obj.is_fold_tab_play">
        <!-- 赛事基础信息 -->
        ...
        <!-- 赛事盘口投注项 -->
        <match-handicap
          :handicap_list="match.other_handicap_list"
          :match="match"
          other_play
        />
        <!-- 视频按钮 -->
...
<script>
  ...
  data(){
    return {
      play_name_list:[],
      // 赛事样式对象
      match_style_obj: _.get(this.match_list_card,`all_card_obj.mid_${this.mid}`,{}),
    }
  },
  computed: {
    // 其他玩法标题
    bet_col() {
          let bet_col = []
          //是否多列
          let multi_column = _.get(this, 'match.tpl_id') == 13
          let play_current_key = _.get(this,'match.play_current_key')
          // 5分钟玩法
          if(play_current_key == 'hps5Minutes'){
              ...
              bet_col = [_.get(this,`match.play_obj.hpid_${hpid}.hpn`,'')]
              if(multi_column){
                bet_col.push('')
              }
            // 波胆
              ...
              let [draw,ht_draw] = this.$root.$t('list.match_tpl_title.tpl0.bold_bet_col')
            ...
            // 15分钟玩法
              ...
              bet_col = [this.$root.$t('list.outright'), '', '', '', '', '']
              if (multi_column) {
                  bet_col.push(...['', '', '', '', '', '', ''])
              }
              // 晋级
          } else if (play_current_key == 'hpsPromotion') {
              bet_col = [this.$root.$t('list.promotion'), '', '', '', '', '']
              if (multi_column) {
                  bet_col.push(...['', '', '', '', '', '', ''])
              }
              //角球
          } else if (play_current_key === 'hpsCorner') {
              bet_col = [...this.$root.$t('list.match_tpl_title.tpl0.corner_bet_col')]
              if (multi_column) {
                  bet_col.push(...[...this.$root.$t('list.match_tpl_title.tpl13_m.corner_col'), '', '', ''])
              }
              //点球
          } else if (play_current_key == 'hpsPenalty') {
              bet_col = [...this.$root.$t('list.match_tpl_title.tpl0.penalty_bet_col')]
              if (multi_column) {
                  bet_col = bet_col.slice(0, 3)
                  bet_col.push(...[...this.$root.$t('list.match_tpl_title.tpl13_m.penalty_col'), '', '', '', '', '', '', '', ''])
              }
              //加时赛
          } else if (play_current_key == 'hpsOvertime') {
              bet_col = [...this.$root.$t('list.match_tpl_title.tpl0.overtime_bet_col')]
              if (multi_column) {
                  bet_col.push(...[...this.$root.$t('list.match_tpl_title.tpl13_m.overtime_col'), '', '', '', '', ''])
              }
              // 罚牌
          } else if (play_current_key == 'hpsPunish') {
              bet_col = [...this.$root.$t('list.match_tpl_title.tpl0.punish_bet_col')]
              if (multi_column) {
                  bet_col.push(...[...this.$root.$t('list.match_tpl_title.tpl13_m.punish_col'), '', '', ''])
              }
          } else {
              bet_col = [...this.$root.$t('list.match_tpl_title.tpl0.bet_col')]
              if (multi_column) {
                  bet_col.push(...['', '', '', '', '', '', ''])
              }
          }
  ...
  watch:{
    ...
    'match.tab_play_keys':{
      handler(tab_play_keys){
        this.set_play_name_list(tab_play_keys)
      },
      immediate: true,
    },
    //赛事阶段变化时跟新次要玩法
    'match.ms'(){
     let tab_play_keys = _.get(this,'match.tab_play_keys', '') || ''
  ...
  methods:{
    ...
    */
    set_play_name_list(tab_play_keys = '') {
      let play_name_list = []
        ...
        let play_name_obj = {
          // 角球
          hpsCorner:{play_name: this.$root.$t('list.corner'), field: 'hpsCorner'},
          // 罚牌
          hpsPunish:{play_name: this.$root.$t('list.punish'), field: 'hpsPunish'},
          // 晋级赛
          hpsPromotion:{play_name: this.$root.$t('list.promotion'), field: 'hpsPromotion'},
          // 冠军
          hpsOutright:{play_name: this.$root.$t('list.outright'), field: 'hpsOutright'},
          // 加时赛
          hpsOvertime:{play_name: this.$root.$t('list.overtime'), field: 'hpsOvertime'},
          // 点球大战
          hpsPenalty:{play_name: this.$root.$t('list.penalty'), field: 'hpsPenalty'},
          // 15分钟玩法
          hps15Minutes:{play_name: this.$root.$t('list.15minutes'), field: 'hps15Minutes'},
          // 波胆
          hpsBold:{play_name: this.$root.$t('list.bold'), field: 'hpsBold'},
          // 5分钟玩法 5minutes_roll
          hps5Minutes:{play_name: this.$utils.get_match_status(_.get(this, 'match.ms'), [110]) == 1 ? this.$root.$t('list.5minutes_roll') : this.$root.$t('list.5minutes'), field: 'hps5Minutes'},
        }
        tab_play_keys = tab_play_keys.split(',')
        tab_play_keys.forEach( key => {
         play_name_obj[key] && play_name_list.push(play_name_obj[key])
        });
        this.play_name_list = play_name_list
    },
    /**
    ...
    */
    set_secondary_bg(index, length){
        let bg_status = false
        if([2,5].includes(length) && index == 1 && _.get(this,'match.play_current_key') !== 'hps5Minutes'){
            bg_status = true
    ...
    */
    play_tab_click(obj){
      ...
      let play_key = this.play_name_list[obj.index].field
      // 切换玩法
      this.match_list_data.switch_other_play(this.match.mid,play_key)
      if (this.match.csid == 1) {
        let zhugeObj = {
          "玩法集名称": this.play_name_list[obj.index].play_name,
          "玩法集ID": '',
          "区域位置": "主列表"
        }
        this.$utils.send_zhuge_event('TY_PC_足球_玩法分类导航_点击', zhugeObj)
      }
