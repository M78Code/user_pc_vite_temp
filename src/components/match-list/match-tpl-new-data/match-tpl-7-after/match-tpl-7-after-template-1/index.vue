    ...
    <div class="match-handicap-item-wrap">
      ...
      <div class="match-handicap-item">
        ...
        <div class="basic-col" :style="`width:${match_list_tpl_size.team_width}px !important;`">
          <basis-info2 v-if="is_mounted && match" :match="match" :is_show_more="score_column_position === 'main'" />
        </div>

        <!-- 赛事盘口投注项 -->
        <match-handicap v-if="match" :handicap_list="match_tpl_info[`template_${match_style_obj.data_tpl_id}`].main_handicap_list" :match="match"
      ...
      <div class="match-handicap-item" v-if="lodash.get(match_tpl_info, 'is_show_cur_handicap')">
        ...
        <!-- 赛事盘口投注项 -->
        <match-handicap v-if="match" :handicap_list="get_cur_handicap_list(match, )" :match="match" is_show_score />
...
import { MatchBasisInfo3FullVersionWapper as BasisInfo3 } from 'src/components/match-list/match-basis-info/template-3/index.js'
