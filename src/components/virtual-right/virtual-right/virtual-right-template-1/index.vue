      ...
      <template v-if="[1002,1011,1010,1009].includes(vsport_ctr.info.csid*1)">
        ...
        <template v-if="vsport_ctr.status != 2">
          <div class="horse-title">
            ...
            <!-- 冠军 -->
            <div class="horse-col">{{i18n.t('list.virtual_match_type_title.type1011.bet_col.0')}}</div>
            <!-- 前二 -->
            <div class="horse-col">{{i18n.t('list.virtual_match_type_title.type1011.bet_col.1')}}</div>
            <!-- 前三 -->
            <div class="horse-col" v-if="vsport_ctr.info.csid !='1009'">{{i18n.t('list.virtual_match_type_title.type1011.bet_col.2')}}</div>
          </div>
...
export default defineComponent({
  ...
  setup(props, evnet) {
    ...
    onMounted(() => {
      ...
      $root.$on(MITT_TYPES.EMIT_UPD_TIME_REFRESH_CMD, timer);
    ...
    onUnmounted(() => {
      $root.$off(MITT_TYPES.EMIT_UPD_TIME_REFRESH_CMD, timer);
    });
    /**
     * @Description 计时器 
