...
<template>
  <div class="basketball-result">
    ...
    <div class="time yb-flex-center" :class="{end:vsport_ctr.basketball_line_width == 100}" v-if="vsport_ctr.info.mmp == 'PREGAME'">
      <div>{{ $t('vsport.period')}}</div>
      <div>{{vsport_ctr.basketball_strat_time}}</div>
    </div>
    <!-- 比赛结束 -->
    <div class="match-end" v-if="vsport_ctr.info.mmp == 'INGAME'">{{ $t('vsport.full_time')}}</div>
    <!-- 比赛进度 -->
    <div class="progress" v-if="vsport_ctr.info.mmp == 'PREGAME' || vsport_ctr.status == 2">
      <div class="text yb-flex-center">
        <div class="col">{{ $t('vsport.start')}}</div>
        <div class="col">{{ $t('vsport.last_minute')}}</div>
        <div class="col">{{ $t('vsport.end')}}</div>
      </div>
      <div class="line-wrap">
...
.basketball-result {
  ...
  .match-end {
    ...
    color: #000;
    font-weight: bold;
    background: url("/yazhou-pc/image/common/virtual/timebg.png") no-repeat;
