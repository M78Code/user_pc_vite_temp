  ...
  <div class="league-rank">
    <template v-if="vsport_ctr.league_rank_list.length > 0">
      <div class="title" :class="{'fixed_league_rank_title': titleFixed}"
        :style="{'top': `${titleTop}px`, 'width': titleFixed ? `${titleWidth}px` : 'auto'}"><span>{{ $t('vsport.rank')}}</span></div>
      <div class="header" ref="league_rank_header">
        <!-- 排名 -->
        <div class="col1">{{ $t('vsport.gtab9')}}</div>
        <!-- 球队 -->
        ...
        <!-- 积分 -->
        <div class="col5">{{ $t('vsport.gtab7')}}</div>
      </div>
    </template>
    <!-- 榜单列表 -->
