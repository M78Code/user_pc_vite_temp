...
<template>
  <div class="group-rank">
    ...
    <div class="header">
      ...
      <!-- 进 -->
      <div class="col5">{{$root.$t('vsport.gtab4')}}</div>
      <!-- 失 -->
      <div class="col6">{{$root.$t('vsport.gtab5')}}</div>
      <!-- 净胜球 -->
      <div class="col7">{{$root.$t('vsport.gtab6')}}</div>
      <!-- 积分 -->
      <div class="col8">{{$root.$t('vsport.gtab7')}}</div>
    </div>
    <!-- 小组 -->
    <div class="group-item" v-for="(item,index) in vsport_ctr.group_rank_list" :key="index">
      <div class="group-name">{{$root.$t('vsport.xgroup').replace('%s',item.groupId)}}</div>
