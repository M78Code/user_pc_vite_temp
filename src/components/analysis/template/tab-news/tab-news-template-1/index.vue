<template>
  ...
  <!-- 封面图片 -->
              <p class="img" :style="{background: `url('${get_file_path(item.thumbnails.split(';')[0]) || '~public/image/yabo/png/article_list_img.png'}') no-repeat left center`}"></p>
              ...
              <div class="article_info">
                <p class="title ellipsis">
                  <!-- 标签 tagColor: "#399FC8" -->
                  <span v-if="item.tagName" class="tag" :style="{'background': `${item.tagColor}`}">{{item.tagName}}</span>
                  {{item.articleTittle}}
                </p>
                <!-- 摘要内容 -->
...
getArticle()
start_article_tiem.value = new Date().getTime()
window.addEventListener('beforeunload', close_win)
window.addEventListener("message", receipt_data)

const receipt_data = ({ data: { name, info } }) => {
  if (name == 'close_win') {
    close_win(info, 1)
  }
}

const close_win = (info, type) => {
  let message = {}
...
const getArticle = () => {
  ...
  api_analysis.getArticlePB(params).then(res => {
    let _data = _.get(res, 'data.data');
    let _code = _.get(res, 'data.code');
    if (_data && (_code == 200)) {
