...
<template>
  <div class="early-settle yb_mt10 yb_mb12" v-if="calc_show || details_show2">
    ...
    <div class="yb_pl10 yb_pt10 yb_mb6" v-if="is_tips_show">
      ...
      <!-- 提前结算可用次数 -->
      <p v-if="item_data.enablePreSettle  && item_data.initPresettleWs && get_user.pcs==1  && get_user.settleSwitch">{{ $t('early.info9')}}：{{ remaining_num }}</p>
    </div>
        ...
        <template v-if="presettleorderdetail_data.length">
          <div class="detail-wrap yb_pb12" v-for="(item,index) in presettleorderdetail_data" :key="index">
            ...
            <p class="order-title row yb_px10">
              ...
              <span>{{(new Date(utils.format_time_zone_time(+item.createTime))).Format(i18n_t('time4'))}}</span>
            </p>
            <!-- 取消原因 -->
            ...
            <template v-if="item.orderStatus == 2">
              <!-- 部分结算 -->
              <!-- 结算本金 -->
              <p class="yb_mt4"><span>{{item.remainingBetAmount?i18n_t('early.list7'):i18n_t('early.list2')}}</span><span>0.00</span></p>
              <!-- 返还金额 -->
            ...
            <template v-else>
              <!-- 部分结算 -->
              <!-- 结算本金 -->
              <p class="yb_mt4"><span>{{item.remainingBetAmount?i18n_t('early.list7'):i18n_t('early.list2')}}</span><span>{{(+item.preBetAmount).toFixed(2)}}</span></p>
              <!-- 返还金额 -->
              ...
              <!-- 剩余本金 -->
              <p v-if="item.remainingBetAmount"><span>{{ $t('early.list3')}}</span><span>{{(+item.remainingBetAmount).toFixed(2)}}</span></p>
            </template>
          </div>
        </template>
      </div>
