...
<template>
  <div class="body-main yb_ml12 yb_pt10" :class="[type_.seriesType == '1'?'yb_pl12':'yb_pl18',{'body-main3':box_bool && index_ == 1,'body-main2':type_.seriesType != '1' && index_ != len-1}]"
    ...
    <div class="row yb_fontsize14 justify-between" :class="type_.seriesType == '1'?'box-top':'box-top2 justify-between'">
      ...
      <p class="text-right begintime" v-if="!type_.acCode&&main.beginTime">{{(new Date(utils.format_time_zone_time(+main.beginTime))).Format(i18n_t('time4'))}}</p>
    </div>
    ...
    <div class="box-bottom row">
      ...
      <!-- <div class="col-8 text-left yb_fontsize10 item-order" v-if="type_.seriesType == '3'">
        <template v-if="!['zh', 'tw'].includes(get_lang)">
          {{(new Date(+type_.orderVOS[0].closingTime)).Format(i18n_t('time7'))}} {{ $t('match_main.cut_off')}}
        </template>
        <template v-else>
          {{(new Date(+type_.orderVOS[0].closingTime)).Format(i18n_t('time7'))}} {{ $t('match_main.cut_off')}}
        </template>
...
// import { i18n_t} from 'src/boot/i18n'
  ...
  // 3个需要特殊对应的国际化数据写到这里
  const i18n_data = ref({
    sport_name: i18n_t(`common_lang.${lang}.sport2`)[main.sportId],
    type: i18n_t(`common_lang.${lang}.matchtype`)[main.matchType],
    mtype: i18n_t(`common_lang.${lang}.odds`)[main.marketType]
  }) 
