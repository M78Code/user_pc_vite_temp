...
-->
<template>
  <div class="activity-layer fullscreen" @click="$emit('activity_hide')">
    <img :src="get_file_path(activity_layerimg)" class="fixed-center img1" @error="handle_img_load_error">
    <img  src="image/wwwassets/activity/colse2.png" class="fixed-center img2" @click.self="$emit('activity_hide')">
...
import lodash from 'lodash'
  ...
  const go_activity = () => {
      let _url = lodash.get(get_banner_obj, 'type4.hostUrl')
      let _type = lodash.get(get_banner_obj, 'type4.urlType')
      if (!_url) return
        ...
        if (/#*\/*details/.test(_url) && $route.name != 'category') {
          ...
          $router.push({ name: 'activity_task', query: { rdm: new Date().getTime() } })
            ...
            if (tid && is_existtid) {
              ...
              $root.$emit(MITT_TYPES.EMIT_HOME_TAB)
            }
          }
      }
      $emit('activity_hide')
