 ...
 * @Date           : 2023-07-15 19:17:43
 * @LastEditors    : lane jstylane@itcom888.com
 * @LastEditTime   : 2023-07-16 19:56:11
 * @FilePath       : \user-pc-vue3\src\public\utils\utils.js
...
import { api_details, api_account } from "src/api/index";
export const utils = {
  // 是否内嵌
  ...
  load_video_resources() {
    ...
    this.timer_load_video = setTimeout(() => {
      api_details.post_video_refer().then((res) => {
        // 获取视频动画域名
        let video_src =
          window.env.config.live_domains[0] ||
          _.get(res, "data.data.referUrl", "");
          ...
          switch (t_path) {
            case "list.match_tpl_title.tpl7.bet_col":
              if (ret && ret[2] && ret[2] == "1X2") {
                if (csid == 2) {
                  //2-篮球
                  ret[2] = window.vue.$root.$t(
                    "list.play_name_other_name.play_capot_name2"
                  );
                }
              }
              break;
            ...
            case "list.match_tpl_title.tpl16.bet_col":
              if (ret) {
                if (csid == 2 || csid == 6) {
                  //2-篮球
                  let cur_title = window.vue.$root.$t(
                    "list.play_name_other_name.play_capot_name2"
                  );
                  ret[0] = cur_title;
                  ret[3] && (ret[3] = ret[3].replace("1x2", cur_title));
                } else if (!(csid == 4 || csid == 1)) {
                  ret[0] = window.vue.$root.$t(
                    "list.play_name_other_name.play_capot_name1"
                  );
                }
              }
            ...
            case "list.match_tpl_title.tpl0.bet_col":
              if (ret) {
                if (csid == 11) {
                  ...
                  ret = _.cloneDeep(
                    window.vue.$root.$t(
                      "list.match_tpl_title.tpl0.bet_col_csid_11"
                    )
                  );
                }
              }
  ...
  is_show_sr_flg(match) {
    ...
    if (
      !(
        window.env &&
        window.env.config &&
        window.env.config.FINAL_TARGET_PROJECT_NAME == "yabo"
      )
    ) {
      // 只有专业版显示sr标志入口图标
      ret = false;
  ...
  zhuge_load_sdk_js(mid = 0) {
    ...
    window.zhuge.load(this.get_zhuge_config_obj().app_key, {
