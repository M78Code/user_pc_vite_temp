 ...
 * @LastEditTime: 2023-08-04 14:50:48
...
class AllDomain {
  ...
  */
  ...
  begin_process_when_use_url_api() {
    ...
    // 获取token
    let token = sessionStorage.getItem("pc_token");
    // 并发请求
    let reqs = [];
    url_api.map((item) =>
      reqs.push(
 ...
 */
  ...
  compute_current_local_api_pool() {
    ...
    new_get_api_obj_arr = _.uniqBy(new_get_api_obj_arr,'api');
    ...
    try {
      ...
      this.compute_current_local_api_pool();
        ...
        if(rejected_num ==  api.length){
          // 失败 页面  没网 之类的 错误页面
          window.vue.$root.$emit('domain_error_alert')
          }else{
      //如果 是检查 域名分组 正确性 并纠错
      if(check_group){
         //  当前  使用的 api
