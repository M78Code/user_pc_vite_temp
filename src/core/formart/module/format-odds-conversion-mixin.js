...
export default {
  ...
  data() {
    return {
      ...
      //   { label: i18n.t('odds.EU'), value: "EU", icon: 'panda-icon-contryEU', id: 1 },//欧洲盘
      //   { label: i18n.t('odds.ID'), value: "ID", icon: 'panda-icon-contryYN', id: 6 },//印尼盘
      //   { label: i18n.t('odds.US'), value: "US", icon: 'panda-icon-contryUS', id: 5 },//美式盘
      //   { label: i18n.t('odds.MY'), value: "MY", icon: 'panda-icon-contryML', id: 3 },//马来盘
      //   { label: i18n.t('odds.GB'), value: "GB", icon: 'panda-icon-contryUK', id: 4 },//英式盘
      //   { label: i18n.t('odds.HK'), value: "HK", icon: 'panda-icon-contryHK', id: 2 },//香港盘
      // ],
      oddsTable: {
        EU: '1',
        HK: '2',
  ...
  methods: {
    compute_value_by_cur_odd_type(val, breakVal, arr, csid) {
      ...
      let str = "";
      breakVal = ""; // 断档值废弃
      let _route = this.$route.name
      this.cur_odd = this.vx_cur_odd;
      // 从欧盘转到港盘
      if (!arr || ['2'].includes(this.oddsTable[this.cur_odd]) && this.vx_cur_odd == 'HK') {
        str = this.calc_odds(val, csid);
        //聊天室跟单特殊处理
        if (arr && arr.includes(this.oddsTable[this.cur_odd]) || this.vx_get_chat_room_type == "HK") {
          str = this.change_EU_HK(str);
        }
        return str;
      }

      if (!arr || arr.includes(this.oddsTable[this.cur_odd]) && this.cur_odd) {
        this.cur_odd == 'EU' ? str = this.calc_odds(val, csid) : str = this.compute_value_by_odd_type(breakVal ? breakVal : val, this.cur_odd, csid);
      } else {
        str = this.calc_odds(val, csid);
      }
      return str;
      // return this.get_accuracy(str);
    },
    ...
    compute_value_by_odd_type(val, odd_type, csid) {
      ...
      let obj = ``;
      let real = "";
      obj = this.vx_odds_coversion_map[`EU_${val}`];
      if (val <= 2.5) {
        // 1	1.01-2.50，以0.01为单位，相应赔率转换
      } else if (val <= 5) {
        ...
        if (!obj) {
          ...
          obj = this.vx_odds_coversion_map[`EU_${real}`];
        }
      } else if (val <= 10) {
