...
## 菜单初入思维
    目前客户端菜单总分为以下4类
        ...
        1.一级菜单 今日滚球等等
        2.二级菜单 一级菜单下常规球种 根据id拼凑请求数据 （例如 今日下足球 1012）
        3.三级菜单 日期（目前只有串关 早盘 电竞有日期列表）
        4.联赛 看项目需求是否需要 

    数据拼装看项目ui设计，菜单元数据接口（yewu11/v3/menu/init），可根据接口返回数据展示对应球种。
##  常规球种ID对应
```javascript
...
## 球种关联
#### 映射关系  (yewu11/v3/menu/loadMapping => Object.h（移动端id）Object.p（pc端id）)
