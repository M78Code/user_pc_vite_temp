	...
	if ([2, 3].includes(menu_root) && !MenuData.is_guanjun()) {
		...
		if (menu_root == 1) {
			...
			if ([50199, 50101].includes(Number(mi))) {
				...
				// 常规赛种、联赛  未开赛
				let nolive_data_match = this.get_match_list_by_mid_for_base_data_res(
					mid,
					csid,
					"nd"
				);
...
const fetch_match_list = (is_socket = false, cut) => {
	...
	// 设置列表接口 和 参数
	let api = api_match['post_fetch_match_list'];
	...
	let send_match_list_request = () => {
		/**返回数据处理************/
		console.log('lockie_test_console', api_match , MenuData.match_list_api_config);
		api && api(_params)
			.then((res) => {
				console.log('lockie_test_console', res);
				// 组件和路由不匹配 菜单id不匹配
				if (
