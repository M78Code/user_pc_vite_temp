...
import { MatchListCardFullVersionWapper as MatchListCard} from '/* webpackChunkName: "pc-mini-chunks" */ "src/components/match-list/match-list-card/index.js"'
...
const match_list = {
	...
	watch: {
		...
		get_unfold_multi_column() {
			this.set_retain_scroll_obj({
				status: true,
				height: this.$matchlist.scroll_top,
			});
		...
		vx_layout_list_type() {
			// 清除过滤条件
			store.dispatch({
				type: 'SET_REMOVE_FILTER_CONDITION',
			})
			// this.vx_set_remove_filter_condition();
		},
		// 联赛筛选
	...
	created() {
		...
		this.check_match_last_update_timer_id = setInterval(
			this.check_match_last_update_time,
			30000
		);
		this.timer_obj = {};
		store.dispatch({
		...
		// 调用列表接口
		useMittOn(MITT_TYPES.EMIT_FETCH_MATCH_LIST, this.fetch_match_list);
		useMittOn(MITT_TYPES.EMIT_API_BYMIDS, this.api_bymids);
		useMittOn(MITT_TYPES.EMIT_MX_COLLECT_MATCH, this.mx_collect_match);
		useMittOn("match_list_show_mids_change", this.show_mids_change);
	},
	methods: {
		...mapActions({
			// 移除选择后初始化
					...
					api(_params)
						.then((res) => {
							...
							if (data.code == 200) {
								...
								this.handle_match_list_request_when_ok(
		...
		api_bymids(
			...
			if (
				...
				(this.$options.name !== "HotMatchList" &&
					["details", "video"].includes(this.$route.name))
			) {
				return;
			}
		...
		set_match_base_info_by_mids_info(match_list, mids_arr, ts1) {
			...
			// 重新计算赛事样式
			this.match_list_card.recompute_match_list_style_obj_and_match_list_mapping_relation_obj_by_matchs(
				mids_arr
			);
			match_list.forEach((match) => {
			...
			if ([2, 3].includes(menu_root) && !MenuData.is_guanjun()) {
				...
				if (menu_root == 1) {
					...
					if ([50199, 50101].includes(Number(mi))) {
						...
						// 常规赛种/联赛  滚球
						let live_data_match = this.get_match_list_by_mid_for_base_data_res(
							mid,
							csid,
							"ld"
						);
	...
	destroyed() {
		...
		clearTimeout(this.get_match_list_timeid);
		// 调用列表接口
		this.$root.$off(MITT_TYPES.EMIT_FETCH_MATCH_LIST, this.fetch_match_list);
		this.$root.$off(MITT_TYPES.EMIT_API_BYMIDS, this.api_bymids);
		this.$root.$off(MITT_TYPES.EMIT_MX_COLLECT_MATCH, this.mx_collect_match);
