...
const match_list = {
	...
	watch: {
		...
		get_unfold_multi_column() {
			store.dispatch({
				type: "SET_RETAIN_SCROLL_OBJ",
				deta: { 
					status: true,
					height: match_scroll_utils.scroll_top, 
				}
			})
			...
			handler(load_data_state) {
				if (this.DOM_ID_SHOW) {
					if (load_data_state == "loading") {
						match_scroll_utils.start_loading();
					} else {
						match_scroll_utils.end_loading();
					}
				}
			},
			immediate: true,
