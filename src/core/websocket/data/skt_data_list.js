...
import {WsSend} from "src/public/utils/ws/wsCtr";   // ws通信接收消息类
import {mapGetters,mapMutations, mapActions} from "vuex";
import UserCtr from 'src/core/user-config/user-ctr.js'
import { project_name, MenuData } from 'src/core'
import ws_debounce from "src/public/mixins/websocket/data/ws_debounce.js"    // ws调用接口节流工具类
export default {
  created() {
    this.ws_debounce = new ws_debounce(this, this.get_match_info_upd)
  },
  data() {
    return {
      change_queue:[],
  ...
  mounted() {
    this.c302_timeout=0;
    //节流方法
    this.get_match_all_list_throttle = this.throttle(
      this.get_match_all_list_throttle,
      (5+parseInt((Math.random()*1000)%6))*1000,
      {
        leading: true,
        trailing: true
      });
  ...
  methods: {
    /**
     * 根据mid获取matchCtr对象中list的赛事对象
     * @param {String} mid 赛事mid
     * @returns
     */
    get_match_obj_from_list(mid){
      let res = {};
      // 列表数据同步
      if(_.get(this.matchCtr,'list.length')){
        for (let i = 0; i < this.matchCtr.list.length; i++) {
          const item = this.matchCtr.list[i];
          // 比对赛事
          if(item.mid == mid){
              res = item;
              break;
     ...
     */
    get_match_all_list_throttle(callback,call_source,mid){
    ...
    RCMD_C101(obj) {
      if(obj && this.matchCtr.match_list_data_sources){
        ...
        if(this.matchCtr.mid_obj[obj.cd.mid]){
          ...
          Object.assign(this.get_match_obj_from_list(skt_data.mid),{ms:skt_data.ms});  
        }
        ...
        try {
          ...
          if(_.get(this.matchCtr,'list.length')){
              for (let i = 0; i < this.matchCtr.list.length; i++) {
                const item = this.matchCtr.list[i];
                // 比对赛事
                if(item.mid == skt_data.mid){
                    // 同步比分
                    Object.assign(item,{...skt_data, handle_time:obj.ctsp});
                    break;
                }
              }
          }
    ...
    // 将新比分合并到旧比分
    combine_msc(msc,new_msc){
      let source_dic = {},new_dic = {},combined = [];
      msc.forEach(m => {
        let code = m.split('|')[0];
        source_dic[code] = m;
      });
    ...
    RCMD_C103(obj) {
      ...
      if(_.get(this.matchCtr,'list.length')){
        for (let i = 0; i < this.matchCtr.list.length; i++) {
          ...
          if(item.mid == skt_data.mid){
          	// 同步比分
            Object.assign(item, {msc:combined});
            break;
          }
        }
      }
    ...
    RCMD_C107(obj) {
      ...
      if(skt_data){
        ...
        Object.assign(this.get_match_obj_from_list(skt_data.mid),skt_data);
      }
    },
    //移除联赛的赛事id,或为空联赛时移除联赛
    remove_mid_of_league(tid,mid){
    ...
    RCMD_C303(obj) {
      if(obj.cd && obj.cd.mid && obj.cd.hpid){
        if(this.is_secondary_oddway(obj.cd.hpid)){
          UserCtr.set_c303_data_change(`${obj.cd.mid}-${obj.cd.hpid}-${Math.random()}`);
          // if(this.run_process_when_need_recompute_container_list_when_scroll){
    ...
    secondary_show_status(cd_data){
      ...
      if(play_id != -1){
        UserCtr.set_corner_oc_change(`${mid}-${status}-${play_id}-${Math.random()}`);
    ...
    RCMD_C113(obj){
      if(obj.cd && obj.cd.mid){
        UserCtr.set_c305_data_change(`${obj.cd.mid}-${Math.random()}`);
        this.run_process_when_need_recompute_container_list_step_three_recompute_next_list_container_top_obj();
      }
    },
    //次要玩法盘口状态变化
    RCMD_C305(obj) {
      if(obj.cd){
        ...
        UserCtr.set_c305_data_change(`${data}-${Math.random()}`);
        this.run_process_when_need_recompute_container_list_step_three_recompute_next_list_container_top_obj();
      }
    },
    //主副盘变更(C304)
      ...
      if(Array.isArray(skt_data) && skt_data.length){
        for (let item of skt_data) {
          if(this.matchCtr.mid_obj[item.mid]){
            ...
            Object.assign(this.get_match_obj_from_list(item.mid),item);
          }
        }
      }
    },
     ...
     * @returns
     */
    SCMD_SPECIAL_C8(status, key='list',paly_key,match) {
      const obj = {};
      obj.key = key;
      obj.module = 'list';
      if(status)
    ...
    // 取消订阅C1
    // calcel_c1_observe(){
    //   WsSend.sktCancelSend(window.ws,WsCmd.S_CMD_MATCH_C8_STATUS);
    // }
