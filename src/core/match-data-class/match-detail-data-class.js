...
export default class MatchInfoCtr {
    ...
    if (list && list instanceof Array) {
      ...
      if (list_normal.length) {
        ...
        for (let i = 0; i < len; i++) {
          for (let k = 0; k < len - i; k++) {
            if (
              parseInt(list_normal[k].hpon) > parseInt(list_normal[k + 1].hpon)
            ) {
              t = list_normal[k];
            ...
            if (item3.ol && item3.ol.length) {
              item3.ol.forEach((item4) => {
                ...
                Object.assign(item4, {
                  ...
                  hid: item3.hid,
                  id_: item3.hn
                    ? `${item.mid}_${item.chpid || item.hpid}_${item4.ot}_${
                        item3.hn
                      }`
                    : item4.oid,
  ...
  setMatchHsStatus(hid, status, ols) {
    if (hid && this.hl_obj[hid]) {
      ...
      if (
        this.hl_obj[hid] &&
        this.hl_obj[hid].ol &&
        this.hl_obj[hid].ol instanceof Array
      ) {
  ...
  updPlay(obj_arr, addPlay) {
    if (obj_arr.length && Array.isArray(obj_arr)) {
      //以前是对象，现在是数组，所以要在外层循环一次
      for (const obj of obj_arr) {
        if (
          obj &&
          this.match_obj &&
          this.match_obj.mid == obj.mid &&
          this.list &&
          ...
          for (let i = 0; i < hps.length; i++) {
            if (
              (!obj.topKey && hps[i].hpid == obj.hpid) ||
              (hps[i].hpid == obj.hpid && hps[i].topKey == obj.topKey)
            ) {
              play = hps[i];
              break;
            }
                    ...
                    item_hl.ol.forEach((item_ol) => {
                      if (item_ol) {
                        ...
                        Object.assign(item_ol, {
                          ...
                          id_: item_hl.hn
                            ? `${obj.mid}_${obj.chpid || obj.hpid}_${
                                item_ol.ot
                              }_${item_hl.hn}`
                            : item_ol.oid,
                        });
