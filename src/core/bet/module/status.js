 ...
 */
      const get_deactive_count = ( ) => {
        let bet_obj,count = 0;
        try{
          bet_obj = BetData.is_bet_single?_.get(BetData,'bet_single_obj'):_.get(BetData,'bet_obj');
...
const has_disable_item = ( ) => {
  ...
  if(BetData.is_bet_single) {
    ...
    index = _.findIndex(BetData.bet_list, (item) => {
      ...
      return [1,2].includes(mhs) || [1,2].includes(hs) || [2,3].includes(active) || BetData.bet_list.length==1;
    });
  }
  return index > -1;
}
