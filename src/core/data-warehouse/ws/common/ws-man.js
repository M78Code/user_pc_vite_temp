 ...
 * @Description: ws通信类
 */
import Ws from "src/core/ws/ws.js";
import WsCmd from "src/core/ws/ws-cmd.js";
export default class wsMan {
  ...
  static run(domain){
    ...
    if(domain){
      ...
      // 获取默认最快域名
      url = window.BUILDIN_CONFIG.domain[window.BUILDIN_CONFIG.current_env][0] || '';
    }
    // 组装ws地址
    url = `${url}/${window.BUILDIN_CONFIG.api.API_PREFIX_WBSOCKET}/push`;
