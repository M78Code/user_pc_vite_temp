...
export const useVirtualMatch = () => {
   ...
   */
  const video_process_init_video = () => {
   ...
   * 赛事初始化字段附加
   */
  const append_init_fields = (f_match) => {
    return {
      ...f_match,
      home:0,
      away:0,
  ...
  /**
    ...
    * @return {Undefined} Undefined
    */
  const get_video_process_by_api = (success_cb,is_no_match_data) => {
   ...
   */
  const set_basketball_video_data = (video_data) => {
  }
  /**
   * 检查是否删除第一期
   */
  const check_is_first_tab_delete = () => {
   ...
   */
  const get_match_result = (mid_str,callback) => {
  ...
  /**
    ...
    * @return {Undefined} Undefined
    */
  const get_video_process_by_api_again = (success_cb) => {
    this.again_total++;
   ...
   * @param {Object} match
   */
  const get_match_video_process = (match) => {
    if(!match){
      return;
    }
    let got_data = false;
  ...
  const set_basketball_score_store = (data,store_key='v-basketball-score') => {
    sessionStorage.setItem(store_key,JSON.stringify(data));
  }
  const get_basketball_score_store = (store_key='v-basketball-score') => {
    let r = null;
