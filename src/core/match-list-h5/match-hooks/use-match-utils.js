...
const useMatchUtils = () => {
   ...
   */
  const match_period_map = (match, has_replace) => {
   ...
   */
  const is_match_playing = () => [1, 2, 7, 10, 110].includes(+ms);
  /**
   * @description: 点击球种前置处理 1. 一键折叠情况下 有联赛tid展开 2. 一键展开情况下 所有联赛tid折叠
   */
  const before_ball_folding_handle = () => {
   ...
   *点击一键收起或者展示时集中修改状态
  */
  const change_ball_icon_state = (csid) => {
    // 当前操作赛种tid列表
    let cur_cid_arr = this.matchCtr.match_list_data_sources.filter(item => item.csid == csid)
    // 赛事折叠状态集合
  ...
  */
   ...
   */
  const combination_matches_list_data = (data) => {
