 ...
 * 赛事页面 hook
 */
import { get_esports_match_by_mids, get_match_base_info_by_mids } from "src/project/api/module/common/index.js";
...
import { last_gcuuid as last_gcuuid_, get_matchs_api_func as get_matchs_api_func_  } from "../composables/match-list-params.js"
import { match_is_empty } from "../composables/match-list-collect.js"
import { onUnmounted, ref } from 'vue'

export const use_match_page = () => {
  const last_gcuuid = ref(last_gcuuid_)
  const get_matchs_api_func = ref(get_matchs_api_func_)
  // 筛选使用到的定时器
  const filter_timer = ref(null)
  // 延时器
  const sub_list_timeout = ref(null)
  //mids接口更新数据时钟
  const update_match_by_mids_timer = ref(null)
  //当前调用的赛事列表接口方法
  const current_invoke_api_func = ref(null)
  //上次调详情的时间点
  const prev_invoke_match_details_time = ref(0)
  ...
  /**
     * 投注框获取到最新赔率同步到列表页
     */
  const bet_odd_synchronize_handle = (data_dict) => {
   ...
   */
  const removeMatch = (callback) => {
    if (callback)
      callback();
    if (!this.matchCtr.list.length) {
      this.match_is_empty.value = true;
    }
  }
  /**
   ...
   * @return {Undefined} Undefined
   */
  const counting_down_start_ended_on = (match_id) => {
  ...
  const footer_event = (obj) => {
    ...
    // 筛选
      ...
      this.scroll_list_wrapper_by(0)
      this.get_match_data_list(() => {
        clearTimeout(filter_timer.value)
        filter_timer.value = setTimeout(() => {
  ...
  const subscription = () => {
    ...
    clearTimeout(sub_list_timeout.value);
    sub_list_timeout.value = setTimeout(() => {
      if (!this.matchCtr) { return; }
      let now = new Date().getTime();
      // 当前时间，减去 上一次的 时间，小于1s，则return 掉，否则，调用订阅赛事推送
      if (now - prev_invoke_match_details_time.value < 1000) {
        return;
      }
      prev_invoke_match_details_time.value = now;
   ...
   * @return {Undefined}
   */
  const update_match_databy_mid = (match_list) => {
  ...
  const get_match_info_upd = (mid, is_subscribe, other) => {
   ...
   * @return {Undefined} Undefined
   */
  const get_match_info_upd_when_other = (mid, is_subscribe, other) => {
    ...
    let fun_temp = () => {
      api_func(params).then(res => {
        ...
        clearTimeout(update_match_by_mids_timer.value);
        this.set_foot_ball_screen_changing(1);
        update_match_by_mids_timer.value = setTimeout(() => {
   ...
   * @return {Undefined} Undefined
   */
  const is_match_playing = (ms) => {
    return [1, 2, 7, 10, 110].includes(ms * 1);
  }
  /**
   * 发起三级菜单请求之前
   */
  const before_load_third_menu_handle = (type) => {
    this.is_data_requesting = true;
  }
  /**
   ...
   * @return {Undefined} Undefined
   */
  const get_match_data_list = (cb) => {


    // 调用列表接口   如  "/v1/m/matchesPB"
    let api_handle_result = this.get_matchs_api_func();
    // 赛事接口调用前置条件处理  1.次要玩法折叠   2 重置到联赛折叠状态； 3. 骨架屏 显示
    this.match_list_api_prev_handle();
    // 新参数直接在此处进行生成新的params
    current_invoke_api_func.value = api_handle_result.api_handle;
    if (current_invoke_api_func.value) {
      //接口调用
      let obj_ = {
        // axios api对象
        axios_api: current_invoke_api_func.value,
        // axios api对象参数
        params: params,
        // 唯一key值
        key: 'list',
        ...
        fun_then: res => {
          ...
          if (this.last_gcuuid.value != res.gcuuid) {
   ...
   */
  const footer_refresh_match_list = () => {
    ...
    let fun_temp = () => {
      if (current_invoke_api_func.value) {
        current_invoke_api_func.value(params).then(res => {
   ...
   * @return {Undefined} Undefined
   */
  const get_match_list_req = (callback, call_source, mid) => {
   ...
   */
  const get_obj = (match_source_data_) => {
  ...
  const csid_same_sort = (data_list, ball_list) => {
    if (!ball_list.length) {
      return data_list
    }
   ...
   * @return {Undefined} Undefined
   */
  const match_status_changed = (state_changed) => {
    const _this = this;
