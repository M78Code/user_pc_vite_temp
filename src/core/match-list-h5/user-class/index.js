...
import { get_file_path } from "../match-utils/get_file_path.js"
...
class userCtr {
   ...
   */
  init() {
    //所有接口上报的 用户信息失效，这里不包含 OSS域名检测的  src\public\utils\allDomain\all_domain.js
  ...
  */
  ...
  set_user_base_info(obj) {
    if (obj) {
      try {
        let data = { languageName: obj.languageName, userMarketPrefer: obj.userMarketPrefer };
  ...
  set_getuserinfo_res(res) {
    ...
    window.vue.$root.$emit(window.vue.emit_cmd.EMIT_SET_GETUSERINFO_OSS_API, oss);
    // 保存userinfo
  ...
  check_if_token_expired_max() {
    ...
    this.token_expired_max_process_timer = setTimeout(() => {
      ...
      if (this.all_expired_count >= this.all_expired_count_max) {
        // 跳转商户
        window.vue.$root.$emit(window.vue.emit_cmd.EMIT_GO_TO_VENDER);
        // 关闭WS
        if (window.ws) {
  ...
  */
  set_web_meta_by_config() {
    ...
    let metas = dom_.getElementsByTagName('meta');
