...
class MatchPage {
  ...
  bet_odd_synchronize_handle(data_dict){
    Object.keys(data_dict).forEach(mid => {
      ...
      MatchCtr.set_match_odd_list(syn_match);
    });
  }
  /**
    ...
    if (match_list && match_list.length) {
      ...
      list.forEach(item => {
        if(item && item.mid)
        {
          ...
          {
            MatchCtr.updMatchInfo(item);
          } else {
            MatchCtr.addMatchInfo(item);
          }
  ...
  get_match_info_upd_when_other(mid,is_subscribe,other){
    ...
    if(!mid || !MatchCtr) return;
    //非赛事列表中的赛事不更新
    if(Array.isArray(mid)){
      let flag = false;
      mid.forEach(_mid => {
        if(MatchCtr.mid_obj[_mid]) flag = true;
      });
      if(!flag && is_subscribe != "is-subscribe")return;
    }else if(!MatchCtr.mid_obj[mid]){
      return;
    }
    //竞足409 不需要euid
    let params = {
    ...
    let fun_temp = ()=>{
      api_func(params).then(res => {
        ...
        if(MatchCtr && MatchCtr.mid_obj && MatchCtr.mid_obj[mid_first] &&
          MatchCtr.mid_obj[mid_first].handle_time &&
      ...
      if(this.current_invoke_api_func){
        this.current_invoke_api_func(params).then(res => {
          ...
          if (lodash.get(data_page, 'length', 0)) {
            ...
            let temp_ = MatchCtr.listComparison(data_page,MatchCtr.match_list_data_sources)
  ...
  get_match_params_detail(params, main_menu_type){
    ...
    if (this.is_in_filtering()) {
      ...
      if(this.get_filter_list != i18n_t('footer_menu.all')){
        // 过滤出 tid（联赛id）
        let current_tids = [];
        for (let f_tid in this.get_filter_list){
    ...
    if(main_menu_type){
      ...
      if(main_menu_type == 3000){
        // 电竞保存csid,否则不用保存csid
        params.csid = this.get_current_esport_csid;
  ...
  match_status_changed(state_changed) {
    ...
    const match_data_list = MatchCtr.list;
