...
import {update_match_parent_card_style} from  "./utils.js"
  ...
  */
  const get_tab_play_height=(mid)=> {
    let { play_current_key,other_handicap_list=[] } = this.match_list_data.mid_obj['mid_' + mid] || {}
  ...
  */
   ...
   * @param {String|Number} mid 赛事id
  */
   update_match_cur_card_style(match ) {
     let {mid} = match
    ...
    if(!card_obj.is_fold_tab_play){
      ...
      this.update_match_parent_card_style(card_obj,this.all_card_obj)
    }
  }
  ...
  */
  ...
  compute_style_template_by_matchinfo_template18(match){
    let cur_match = this.match_list_data.mid_obj[`mid_${match.mid}`]  || {main_handicap_list:[]}
    // 附加盘口高度
