...
class infoUpload {
   ...
   */
    async upload_url_info(params) {
        try {
            let { data } = await this.send_request({ method: 'post', url: "/url_info/createOrUpdate", params })
            if (data) {
                console.log('URL上报成功');
            } else {
                console.log('URL上报失败');
   ...
   */
    async upload_tablet_comput(params) {
        try {
            let { data } = await this.send_request({ method: 'post', url: "/tablet_comput/create", params })
            if (data) {
                console.log('平板上报成功');
            } else {
                console.log('平板上报失败');
